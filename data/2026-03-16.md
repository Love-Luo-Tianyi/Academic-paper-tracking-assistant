...
TL;DR: 提出GenEval方法，结合视觉语言模型和人类知识，通过LoRA适配提升单源域泛化能力，在糖尿病视网膜病变和癫痫发作区检测任务上取得显著效果


<details>
  <summary>Details</summary>
Motivation: 解决跨域图像分类中的泛化难题，特别是在糖尿病视网膜病变分级和癫痫发作区检测等关键任务中。当域间存在未知因果因素差异时，缺乏客观评估这些差异的方法，且通常无法获取数据收集者的元数据或协议级信息。

Method: 1. 提出域一致性边界理论框架评估域间未知因果因素差异；2. 提出GenEval方法，结合基础视觉语言模型和人类知识，通过低秩适配技术桥接因果差距，增强单源域泛化能力。

Result: 在8个糖尿病视网膜病变数据集和2个癫痫发作区检测数据集上，GenEval取得最佳单源域泛化性能：平均准确率分别为69.2%和81%，分别比最强基线高出9.4%和1.8%。

Conclusion: GenEval方法有效解决了跨域图像分类中的泛化问题，特别是在医疗影像分析领域，通过结合视觉语言模型和人类知识显著提升了单源域泛化性能。
...
TL;DR: ABRA是一种针对开放词汇目标检测的域适应方法，能够在没有目标域标注数据的情况下，将源域的类别特定检测知识迁移到目标域，通过几何传输问题在权重空间中对齐源域和目标域专家。


<details>
  <summary>Details</summary>
Motivation: 现有的开放词汇目标检测架构（如Grounding DINO）在域偏移下性能显著下降，而许多实际应用领域（如夜间或雾天场景）缺乏大规模标注数据集，无法直接进行微调。

Method: ABRA将域适应问题建模为预训练检测器权重空间中的几何传输问题，通过对齐源域和目标域专家来传输类别特定知识，实现在没有目标域训练图像的情况下进行知识迁移。

Result: 在多个具有挑战性的域偏移场景下的广泛实验表明，ABRA能够在多种恶劣条件下成功传输类别级别的专业化知识。

Conclusion: ABRA提供了一种有效的域适应方法，能够在没有目标域标注数据的情况下，将源域的类别特定检测知识成功迁移到目标域，解决了开放词汇目标检测在域偏移下的性能下降问题。
...
TL;DR: Surg-R1是一个手术视觉语言模型，通过分层推理和四阶段训练流程，在手术场景理解任务上超越现有模型，在公开基准和外部验证数据集上均表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有手术视觉语言模型缺乏可解释的推理链，而通用推理模型又缺乏领域专业知识，无法处理复杂的手术组合任务。需要开发一个既能提供准确预测又能生成可验证推理的手术专用模型。

Method: 提出三层推理层次结构（感知基础、关系理解、上下文推理），构建了最大的手术思维链数据集（32万推理对），采用四阶段训练流程：监督微调、组相对策略优化和迭代自我改进。

Result: 在SurgBench（6个公开基准+6个多中心外部验证数据集）上评估，Surg-R1获得最高Arena Score（64.9%），优于Gemini 3.0 Pro（46.1%）和GPT-5.1（37.9%），在大多数任务上超越专有推理模型和专用手术VLM，外部验证上比最强手术基线提升15.2个百分点。

Conclusion: Surg-R1通过分层推理架构和系统化训练流程，实现了手术场景理解的可解释性和准确性，为手术AI系统提供了可验证的推理能力，在多个任务和数据集上展现了优越性能。
...
TL;DR: 本文重新审视了模型缝合技术，研究异质视觉基础模型（VFMs）是否可缝合，提出了系统化缝合协议，发现通过目标模型倒数第二层的特征匹配损失可实现可靠缝合，并提出了VFM缝合树用于多模态LLMs的精度-延迟权衡。


<details>
  <summary>Details</summary>
Motivation: 传统模型缝合研究主要关注相同数据集训练的模型，但视觉基础模型（VFMs）在目标、数据和模态混合方面存在显著差异。本文旨在探索异质VFMs是否可缝合，以整合互补优势并理解其表示对齐或分歧的位置。

Method: 提出了系统化缝合协议，涵盖缝合点、缝合层家族、训练损失和下游任务。特别发现使用目标模型倒数第二层的简单特征匹配损失可实现可靠缝合。基于此进一步提出了VFM缝合树（VST），共享早期层同时保留后期层。

Result: 1) 缝合层训练至关重要，传统方法在浅层缝合点难以保持精度；2) 使用目标模型倒数第二层的特征匹配损失，异质VFMs可在视觉任务中可靠缝合；3) 深层缝合点的缝合模型可以超越任一组成模型，仅需少量推理开销；4) VST实现了可控的精度-延迟权衡。

Conclusion: 本研究将模型缝合从诊断性探针提升为整合互补VFM优势的实用方法，能够精确定位其表示对齐或分歧的位置，并为多模态LLMs提供了有效的精度-延迟权衡解决方案。
...
<details>
  <summary>Details</summary>
Motivation: HRCT诊断报告生成面临高病理多样性和3D空间稀疏性的挑战，现有视频语言模型在医疗领域的适应性尚未充分探索，需要专门框架来提升诊断精度

Method: 提出AbSteering框架：1）异常中心思维链方案强制异常推理；2）直接偏好优化目标利用临床易混淆异常作为硬负样本来增强细粒度判别能力

Result: 通用视频语言模型在该框架下展现出强大的医学影像迁移能力，超越预训练的大规模CT基础模型，实现更高的检测灵敏度同时减少幻觉

Conclusion: AbSteering证明通用视频语言模型通过适当引导可有效应用于高容量医学影像分析，为HRCT报告生成提供了新的有效范式
...
TL;DR: GDO框架通过计算六个样本描述符来优化多模态指令调优的数据选择，在固定训练协议下用更少样本实现更快收敛和更高准确率。


<details>
  <summary>Details</summary>
Motivation: 多模态指令调优通常计算效率低下，因为训练资源分散在大量混合图像-视频数据上，而这些数据的效用差异很大。需要一种能根据目标优化数据选择的方法。

Method: 提出目标驱动数据优化(GDO)框架，为每个候选样本计算六个描述符，然后为不同目标构建优化的1×训练子集。在固定的一轮Qwen3-VL-8B-Instruct训练和评估方案下进行实验。

Result: 相比512k样本的Uni-10x基线，GDO在MVBench、VideoMME、MLVU和LVBench上分别只需35.4k、26.6k、27.3k和34.7k样本就能达到基线性能，同时准确率分别提升+1.38、+1.67、+3.08和+0.84个百分点。在强调时序性的设置下，长视频理解能力稳步提升。
...
TL;DR: CalliMaster是一个统一的书法页面生成框架，通过解耦空间规划和内容合成来解决现有方法在字形精度和布局构成之间的平衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有字符模型缺乏空间上下文，而页面级方法往往牺牲笔触细节。需要解决书法页面合成中字形精度与布局构成之间的平衡问题。

Method: 采用"规划后书写"的人类认知过程，提出粗到细的管道（文本→布局→图像）。使用单一多模态扩散变换器，空间规划阶段预测字符边界框建立全局空间排列，然后作为几何提示用于内容合成阶段，通过流匹配渲染高保真笔触。

Result: 实现了最先进的生成质量，解耦设计支持多种下游功能：可控语义重新规划（调整字符大小或位置时自动协调周围空白和笔势），并扩展到文物修复和法证分析。

Conclusion: CalliMaster通过解耦空间规划和内容合成，为数字文化遗产提供了一个全面的工具，解决了书法页面合成中的关键挑战。
...
TL;DR: 本文提出通过精确的设备特定退化建模来改进智能手机数字变焦中的RAW图像超分辨率性能，相比使用任意选择的退化模型，该方法在真实数据上表现更优。


<details>
  <summary>Details</summary>
Motivation: 智能手机数字变焦依赖基于学习的超分辨率模型处理RAW传感器图像，但获取传感器特定的训练数据困难，因为缺乏真实高分辨率图像。现有的"反处理"合成数据生成方法存在域差距问题，由于退化建模不完整或不真实。

Method: 通过校准建模设备特定的退化，将公开可用的渲染图像反处理到不同智能手机的RAW域，使用这些图像对训练单图像RAW-to-RGB超分辨率模型，并在保留设备上进行评估。

Result: 实验表明精确的退化建模带来显著改进，我们的超分辨率模型优于在任意选择的退化池上训练的基线模型。

Conclusion: 原则性和精心设计的退化建模可以增强真实世界条件下的超分辨率性能，设备特定的退化建模比通用的退化假设更有效。
...
Method: 利用T2I偏好基准，直接从初始噪声和给定提示预测图像数值质量，选择高质量噪声输入扩散模型生成

Result: Naïve PAINE在多个提示语料库基准测试中优于现有方法

Conclusion: Naïve PAINE能有效提高扩散模型生成质量，轻量级设计可无缝集成到现有扩散模型流程中
...
<details>
  <summary>Details</summary>
Motivation: 现有的自回归扩散模型使用滑动窗口缓存时会丢弃过去上下文，导致长视频生成中出现保真度下降、身份漂移和运动停滞。当前方法固定保留早期令牌作为注意力锚点，但静态锚点无法反映视频内容的动态演变。

Method: MemRoPE包含两个协同设计的组件：1) 记忆令牌通过指数移动平均将所有过去的键连续压缩为双流（长期和短期），在固定大小缓存中保持全局身份和近期动态；2) 在线RoPE索引缓存未旋转的键，在注意力时动态应用位置嵌入，确保聚合不受冲突位置相位影响。

Result: 大量实验验证MemRoPE在时间连贯性、视觉保真度和主题一致性方面优于现有方法，支持分钟到小时级别的视频生成。
...
<details>
  <summary>Details</summary>
Motivation: 急诊放射学中腹部CT扫描的创伤性损伤准确检测和定位面临重大挑战，主要原因是标注医学数据严重稀缺，需要开发标签高效的方法来解决这一问题。

Method: 采用基于补丁的掩码图像建模(MIM)在1,206个无标注CT体积上预训练3D U-Net编码器，学习鲁棒的解剖表示。预训练编码器支持两个下游临床任务：使用VDETR和顶点相对位置编码的3D损伤检测，以及多标签损伤分类。在检测任务中，采用2,000个未标注体积的半监督学习和一致性正则化。
...
Conclusion: 自监督预训练与半监督学习相结合能有效解决医学成像中的标签稀缺问题，在有限标注下实现鲁棒的3D目标检测，证明了自监督特征的即时可迁移性。
...
TL;DR: SERA提出了一种用于指称图像分割的时空专家路由架构，通过表达感知的轻量级专家细化解决现有方法在空间一致性和边界精度方面的不足。


<details>
  <summary>Details</summary>
Motivation: 指称图像分割旨在为自然语言描述生成像素级掩码。尽管预训练视觉语言模型改进了语义理解，但现有方法通常采用统一的细化策略，无法匹配指称表达的多样化推理需求，导致预测结果出现碎片化区域、不准确边界甚至错误对象等问题。

Method: SERA在视觉语言框架内引入两个互补阶段的轻量级表达感知专家细化：1) SERA-Adapter在选定骨干块中插入表达条件适配器，通过专家引导细化和跨模态注意力提升空间一致性和边界精度；2) SERA-Fusion将令牌特征重塑为空间网格并应用几何保持专家变换来增强中间视觉表示。此外，轻量级路由机制自适应加权专家贡献，同时保持与预训练表示的兼容性。

Result: 在标准指称图像分割基准测试中，SERA始终优于强基线方法，在需要准确空间定位和精确边界描绘的表达上取得了特别明显的增益。

Conclusion: SERA通过表达感知的专家细化架构有效解决了指称图像分割中的空间一致性和边界精度问题，同时通过参数高效调优策略（仅更新归一化和偏置项）保持计算效率，影响不到1%的骨干参数。
...
TL;DR: 研究发现当前视觉语言模型在空间推理方面存在系统性缺陷，主要源于CLIP风格图像编码器和1D位置编码的设计选择，这些缺陷无法通过现有改进完全解决。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在通用基准测试中表现良好，但在理解2D空间关系（如相对位置、布局和计数）方面仍然脆弱。作者认为这种失败不仅仅是数据问题，更与当前VLM流水线中的主导设计选择密切相关。

Method: 在LLaVA框架内进行控制性诊断研究，评估前沿模型和LLaVA变体在一套空间基准测试上的表现，比较基于CLIP的编码器与使用更密集或生成目标的替代方案，以及增强2D位置编码的变体。

Result: 结果显示不同模型之间存在一致的空间性能差距，表明编码器目标和位置结构会影响空间行为，但无法完全解决空间推理问题。
...
TL;DR: AccelAes是一个无需训练的框架，通过美学感知的时空缩减加速扩散变换器（DiTs），同时提升感知美学质量。它利用提示语义和交叉注意力信号构建美学焦点掩码，重新分配计算到关键区域，并使用轻量级步级预测缓存减少时间冗余。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器（DiTs）在高分辨率文本到图像生成中表现出色，但密集空间令牌的二次自注意力导致高推理延迟，限制了部署。研究发现去噪过程在美学描述符方面具有空间非均匀性，美学相关区域需要集中计算，而低亲和力区域存在冗余计算。

Method: 提出AccelAes框架：1）基于提示语义和交叉注意力信号构建一次性美学焦点掩码（AesMask）；2）在局部计算可行时，使用SkipSparse重新分配计算和引导到掩码区域；3）使用轻量级步级预测缓存定期替换完整的Transformer评估，减少时间冗余。

Result: 在代表性DiT家族上的实验显示，AccelAes实现了持续加速并改善了美学导向的质量。在Lumina-Next上，AccelAes实现了2.11倍加速，ImageReward评分比密集基线提高了+11.9%。

Conclusion: AccelAes通过美学感知的时空缩减有效加速扩散变换器，同时提升感知美学质量，为高分辨率文本到图像生成提供了实用的加速解决方案。
...
Conclusion: MRGeo首次系统性地解决了跨视角地理定位在损坏环境下的鲁棒性问题，通过特征增强和几何约束的双重策略，显著提升了模型在真实世界应用中的实用性和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在金融、医疗等关键领域的广泛应用带来了严重的安全和隐私风险。现有隐私保护方法在泛化性和非破坏性方面存在局限，难以处理未见过的隐私查询且可能降低模型标准任务性能。

Method: Neural Gate通过神经元级模型编辑来增强隐私保护。方法学习特征向量来识别与隐私相关概念相关的神经元，然后精确指导模型参数更新，提高模型对隐私问题的拒绝率。

Result: 在MiniGPT和LLaVA上的实验表明，该方法显著提升了模型的隐私保护能力，同时保持了原始效用。能够将隐私保护行为扩展到编辑过程中未见过的新敏感查询。

Conclusion: Neural Gate提供了一种有效的方法来缓解大型视觉语言模型的隐私风险，通过神经元级编辑在增强隐私保护的同时保持模型性能，解决了现有方法在泛化性和非破坏性方面的限制。
...
<details>
  <summary>Details</summary>
Motivation: 受人类观察快速移动物体（如鸟类、昆虫）时通过预测其未来位置来清晰感知的生物学机制启发，旨在将预测与感知相结合，提升自动驾驶等场景中3D物体感知的准确性和效率。

Method: 提出预测即感知（PAP）框架，包含预测和感知两个主要模块。预测模块基于当前帧感知结果预测自我车辆和周围交通参与者的未来位置，这些预测位置作为查询传递给下一帧的感知模块，感知结果再迭代反馈给预测模块，形成闭环。

Result: 在nuScenes数据集上使用端到端模型UniAD进行评估，结果显示PAP结构将目标跟踪准确率提升了10%，推理速度提高了15%。

Conclusion: 这种仿生设计显著提升了感知模型的效率和准确性，同时减少了计算资源消耗，证明了预测与感知整合在3D物体感知任务中的有效性。
...
Method: 创建了A2Z数据集，包含100万个ABC CAD模型，提供高分辨率网格、3D手绘草图、BRep几何拓扑信息（共边、角点、表面）、文本描述和标签。使用新颖指标、GPT-5、Gemini和人工反馈评估数据质量。训练基础模型在15万个CAD模型子集上检测BRep共边和角点。

Result: 构建了迄今为止最大的多模态CAD标注数据集（近5TB存储），包含1000万条标注。将25,000个专业设计的电子外壳CAD模型合并到数据集中。训练的基础模型能够从3D扫描中检测BRep共边和角点顶点。

Conclusion: A2Z数据集为CAD学习/检索任务提供了前所未有的多模态标注资源，支持CAD逆向工程中的关键下游任务。数据集、评估指标和模型检查点将公开发布，推动相关研究发展。
...
Method: 提出D-Negation数据集，包含正负语义描述的对象标注；设计分组对立学习框架，将对立语义描述组织成结构化组，并制定两个互补的损失函数来鼓励模型进行否定和语义限定符推理。

Result: 将数据集和学习策略集成到最先进的基于语言的定位模型中，仅微调不到10%的参数，在正向和负向语义评估上分别获得高达4.4 mAP和5.7 mAP的提升。

Conclusion: 明确建模否定语义可以显著增强视觉语言定位模型的鲁棒性和定位准确性，为解决复杂语义推理问题提供了有效途径。
...
TL;DR: RoboStereo是一个对称双塔4D世界模型，通过双向跨模态增强确保时空几何一致性，并基于此提出了首个统一的世界模型策略优化框架，在精细操作任务上实现>97%的平均相对改进。
...
Method: 提出RoboStereo对称双塔4D世界模型，采用双向跨模态增强确保几何一致性；基于此构建统一策略优化框架，包括测试时策略增强、模仿进化策略学习和开放探索策略学习三个组件。
...
Method: 提出LiDAR反射率引导的显著高斯溅射方法：1）从LiDAR提取几何和反射率特征点初始化结构感知的显著高斯表示；2）通过显著变换和改进的密度控制细化高斯以捕捉边缘和平面结构；3）将LiDAR强度校准为反射率并附加到每个高斯作为光照不变的材料通道；4）与RGB联合对齐以增强边界一致性。

Result: 在Waymo Open Dataset上的实验表明，LR-SGS以更少的高斯数和更短的训练时间实现了优越的重建性能。特别是在复杂光照场景中，PSNR比OmniRe高出1.18 dB。

Conclusion: LR-SGS通过充分利用LiDAR点云的几何和反射率信息，结合改进的高斯表示和密度控制，有效解决了自动驾驶场景中复杂光照和高速运动带来的挑战，实现了更鲁棒和高效的重建。
...
<details>
  <summary>Details</summary>
Motivation: 标准GRPO框架在评估一组生成样本时，仅使用单一条件进行对比，导致样本间关系探索不足，限制了偏好对齐的效果和性能上限。
...
<details>
  <summary>Details</summary>
Motivation: 传统世界模型主要关注光度细节而忽略几何一致性，导致预测结果在几何上不一致。需要一种专注于几何演化的高效预测方法

Method: 利用冻结VGGT的潜在token作为世界状态，训练轻量级时序流transformer自回归预测特征轨迹。采用z-prediction参数化解决高维特征空间中的流匹配问题，以及两阶段潜在流强制课程学习缓解自回归滚动的暴露偏差
...
TL;DR: VFMRecon：首个将可迁移视觉基础模型先验与尺度一致性要求结合的场景级神经重建方法，通过尺度对齐和轻量适配器实现跨域鲁棒重建


<details>
  <summary>Details</summary>
Motivation: 单目视频的场景级神经体积重建面临严重域偏移挑战，现有视觉基础模型提供可迁移先验但尺度模糊预测与体积融合的尺度一致性要求不兼容

Method: 1) 轻量级尺度对齐阶段恢复多视角尺度一致性；2) 通过轻量任务特定适配器将预训练VFM特征集成到神经体积重建流程中，保持跨域鲁棒性

Result: 在ScanNet测试集、TUM RGB-D和Tanks and Temples数据集上均达到最先进性能，在挑战性户外Tanks and Temples数据集上F1分数70.1，远超VGGT的51.8

Conclusion: VFMRecon成功桥接了可迁移视觉基础模型先验与尺度一致性要求，在场景级神经重建中实现了跨域鲁棒性和优异性能
...
TL;DR: AVION是一个针对遥感图像的知识蒸馏框架，通过教师模块构建语义丰富的文本原型，学生模块集成轻量级可学习提示，提升视觉语言模型在遥感场景中的适应能力。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在遥感图像适应上面临两个主要挑战：文本表示的语义覆盖有限，以及视觉特征的适应性不足。这在涉及多种视觉外观和细粒度物体区分的航空场景中尤为显著。

Method: AVION采用知识蒸馏框架，教师模块从大语言模型收集描述并利用遥感图像特征验证有效性来构建语义丰富的文本原型；学生模块在视觉和语言编码器中集成轻量级可学习提示，在教师指导下对齐嵌入及其跨模态关系。训练后学生模块可独立推理。

Result: 在六个光学遥感基准测试中，AVION提高了少样本分类和基础类准确率，同时不损害对新类别的泛化能力。它还增强了跨模态检索的平均召回率，且仅需极少的额外可训练参数。

Conclusion: AVION通过知识蒸馏框架有效解决了视觉语言模型在遥感图像适应中的语义覆盖和特征适应性问题，在多个遥感任务上取得了显著性能提升。
...
### [28] [MoKus: Leveraging Cross-Modal Knowledge Transfer for Knowledge-Aware Concept Customization](https://arxiv.org/abs/2603.12743)
*Chenyang Zhu,Hongxiang Li,Xiu Li,Long Chen*

Main category: cs.CV

TL;DR: MoKus框架通过跨模态知识转移实现知识感知的概念定制，将多样化的文本知识绑定到目标视觉概念，超越传统基于稀有令牌的方法


<details>
  <summary>Details</summary>
Motivation: 传统概念定制方法依赖稀有令牌绑定目标概念，但存在性能不稳定问题，且稀有令牌无法传达目标概念的内在知识。需要开发能够绑定多样化文本知识到视觉概念的新方法

Method: 提出MoKus框架，包含两个阶段：1) 视觉概念学习阶段学习锚定表示存储目标概念的视觉信息；2) 文本知识更新阶段将知识查询的答案更新为锚定表示，实现高保真定制生成。基于跨模态知识转移的观察

Result: MoKus在提出的KnowCusBench基准测试中优于现有最先进方法，并能扩展到虚拟概念创建、概念擦除等知识感知应用，在世界知识基准测试上也显示出改进能力

Conclusion: MoKus框架通过跨模态知识转移有效解决了知识感知概念定制任务，实现了文本知识到视觉概念的高质量绑定，为概念定制领域提供了新的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 骨料作为建筑材料的主要骨架，其尺寸和形态信息对质量控制至关重要。现有的3D扫描方法（如激光扫描或CT）设备昂贵，难以在生产现场应用，需要一种低成本、灵活的3D重建方法。

Method: 提出基于标记的摄影测量方法，通过背景抑制、点云拼接和尺度参考，实现高质量的骨料三维模型重建。该方法成本低、灵活性高。

Result: 重建结果的准确性通过地面真实数据验证，对选定骨料样本进行了2D和3D形态特性比较分析，发现两者统计结果存在显著差异。

Conclusion: 该方法能够以低成本、便捷地获取骨料的3D形态信息，便于骨料检测、数据收集和3D形态分析，具有实际应用价值。
...
TL;DR: V3Fusion提出了一种基于视觉和语言双模态的VLM融合方法，通过焦点误差多样性和CKA焦点多样性度量模型互补性，使用遗传算法优化模型组合，在多个基准测试中显著超越单个最佳VLM性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要探索基于语言的VLM集成和协作技术，但缺乏利用视觉和语言双模态进行多样化模型选择的方法。需要一种能够动态捕捉认知不确定性并缓解幻觉的融合方法。

Method: 提出V3Fusion方法：1）引入焦点误差多样性捕捉VLM间的互补推理；2）使用CKA焦点多样性度量视觉嵌入的差异性；3）在候选VLM池构建的集成表面上应用遗传算法，剔除对融合性能无贡献的组件模型；4）为每个任务识别最佳组合并融合输出。

Result: 在四个主流VLM基准测试中表现优异：在MMMU上比最佳VLM提升8.09%准确率，在MMMU-Pro上提升4.87%；在生成任务上超越Intern-VL2-8b和Qwen2.5-VL-7b等顶级VLM；即使在无多数共识或多数VLM预测错误时也能产生高性能的双焦点多样性融合预测。

Conclusion: V3Fusion通过视觉和语言双模态的多样化模型选择，实现了异构VLM的有效融合，能够动态捕捉认知不确定性并缓解幻觉问题，在多个视觉语言推理任务中显著提升性能。
...
### [31] [STRAP-ViT: Segregated Tokens with Randomized -- Transformations for Defense against Adversarial Patches in ViTs](https://arxiv.org/abs/2603.12688)
...
<details>
  <summary>Details</summary>
Motivation: 对抗性补丁能够劫持ViT的自注意力机制，将注意力集中在小的、高对比度区域，并破坏类别token以强制错误分类。研究发现包含对抗性噪声的token具有不同的统计特性。

Method: 提出STRAP-ViT机制：1) 检测阶段使用Jensen-Shannon散度作为度量来分离异常token；2) 缓解阶段对异常token应用随机复合变换，使对抗性噪声失效。该方法作为不可训练的即插即用模块集成到ViT架构中。

Result: 在多个预训练ViT架构（ViT-base-16和DinoV2）和数据集（ImageNet和CalTech-101）上测试，针对多种对抗攻击（Adversarial Patch、LAVAN、GDPA和RP2），STRAP-ViT提供了优秀的鲁棒精度，在干净基线的2-3%范围内，并优于最先进方法。
...
TL;DR: 本文提出了一种基于预训练EfficientNet情感识别模型的快速方法，用于ABAW竞赛中的帧级面部情感理解任务，并通过滑动窗口平滑和暴力检测任务的帧嵌入聚合来提升性能。


<details>
  <summary>Details</summary>
Motivation: 参加第10届ABAW竞赛，需要解决帧级面部表情识别、效价-唤醒度估计、动作单元检测等任务，以及细粒度的暴力检测任务，目标是提高在AffWild2数据集上的验证指标。

Method: 1. 使用预训练的EfficientNet情感识别模型提取面部嵌入特征；2. 当模型置信度超过阈值时直接使用其预测，否则将嵌入特征输入到在AffWild2数据集上训练的多层感知机；3. 通过固定大小的滑动窗口平滑帧级预测分数以减少噪声；4. 对于暴力检测任务，探索多种预训练架构提取帧嵌入特征并进行聚合用于视频分类。

Result: 在ABAW挑战的四个任务上，该方法显著提升了验证指标，优于现有基线方法。

Conclusion: 提出的基于预训练EfficientNet模型和简单后处理的方法在ABAW竞赛的多项情感理解任务中表现优异，验证了该方法的有效性。
...
### [33] [VCBench: A Streaming Counting Benchmark for Spatial-Temporal State Maintenance in Long Videos](https://arxiv.org/abs/2603.12703)
*Pengyiang Liu,Zhongyue Shi,Hongye Hao,Qi Fu,Xueting Bi,Siwei Zhang,Xiaoyang Hu,Zitian Wang,Linjiang Huang,Si Liu*

Main category: cs.CV

TL;DR: VCBench是一个流式计数基准测试，将计数重新定位为诊断视频理解中世界状态维护能力的探针，包含对象计数和事件计数两个维度，共8个子类别，通过流式多时间点查询评估模型的状态维护能力。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解基准测试在多维度评估方面有所进展，但对模型如何维护世界状态的观察仍不足。视频理解需要模型在播放过程中持续跟踪和更新世界状态，因此需要专门的基准来诊断这种能力。

Method: 将计数重新定位为最小探针来诊断世界状态维护能力，将其分解为对象计数（跟踪当前可见对象 vs. 跟踪累积唯一身份）和事件计数（检测瞬时动作 vs. 跟踪完整活动周期），形成8个细分子类别。构建包含406个视频的数据集，带有帧级标注，生成1000个流式QA对，通过流式多时间点查询观察状态维护轨迹。

Result: 评估主流视频-语言模型显示，当前模型在时空状态维护方面仍存在显著缺陷，特别是在周期性事件计数等任务上表现不佳。设计了三个互补指标来诊断数值精度、轨迹一致性和时间意识。

Conclusion: VCBench为测量和改进视频理解系统中的状态维护能力提供了一个诊断框架，揭示了当前模型在维护世界状态方面的局限性，并为未来模型改进提供了方向。
...
TL;DR: UNIStainNet：一种基于病理基础模型UNI的虚拟免疫组化染色方法，从H&E图像生成多种IHC标记物染色，单模型支持多标记物，在MIST和BCI数据集上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 从常规H&E图像进行虚拟IHC染色可加速诊断，减少组织切片需求。现有方法通过对比目标、原型匹配或域对齐提高真实性，但生成器缺乏病理基础模型的直接指导。

Method: 使用基于SPADE-UNet的架构，以冻结病理基础模型UNI的密集空间token为条件，提供组织级语义指导。采用错位感知损失套件保持染色定量准确性，学习染色嵌入使单模型支持多种IHC标记物。
...
Conclusion: UNIStainNet通过病理基础模型的语义指导实现了高质量的虚拟IHC染色，单模型支持多标记物，为组织有限情况下的诊断提供了有效解决方案。
...
TL;DR: 本文针对文档布局分析（DLA）领域，提出了新的评估方法：结构语义单元（SSU）和COTe评分，以解决传统目标检测指标在二维文档分析中的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统DLA评估使用IoU、F1或mAP等通用目标检测指标，但这些指标是为3D空间投影到2D图像设计的，不适合原生二维的印刷媒体。这种不匹配导致模型性能评估可能产生误导或信息不足。

Method: 提出两种创新方法：1）结构语义单元（SSU）- 一种关系标注方法，将焦点从物理结构转向语义结构；2）COTe评分（覆盖率、重叠度、侵入度、冗余度）- 一种可分解的页面解析质量度量指标。

Result: 在3个DLA数据集上评估5个常见DLA模型，发现COTe评分比传统指标更具信息性，能揭示不同模型的独特失败模式（如突破语义边界或重复解析同一区域）。COTe评分将解释-性能差距相对F1减少了高达76%。即使没有显式SSU标注，COTe的粒度鲁棒性也基本保持。

Conclusion: SSU和COTe评分为DLA提供了更稳健、可比较和细致的评估框架。作者发布了SSU标注数据集和Python库，以促进这些方法在DLA项目中的应用。
...
Method: 提出IGASA框架，基于分层金字塔架构进行鲁棒的多尺度特征提取和融合。包含两个关键组件：分层跨层注意力模块（通过跳跃注意力机制对齐多分辨率特征，增强局部几何一致性）和迭代几何感知细化模块（在粗匹配阶段建立的可靠对应关系基础上进行精细匹配）。

Result: 在3D(Lo)Match、KITTI和nuScenes四个广泛认可的基准数据集上进行评估，实验结果表明IGASA显著超越最先进方法，在配准精度上实现了显著改进。
...
TL;DR: CognitionCapturerPro是一个增强型框架，通过整合EEG与多模态先验（图像、文本、深度和边缘）来改进视觉刺激重建，显著提升了检索精度。
...
Conclusion: CognitionCapturerPro通过多模态先验整合和不确定性加权机制，有效解决了EEG视觉重建中的保真度问题，取得了显著的性能提升。
...
TL;DR: 论文提出了Dyn-Bench基准测试，用于评估多模态大语言模型在动态4D场景中的时空推理和动态物体定位能力，发现现有模型在时空推理和动态物体定位之间存在性能矛盾，并提出结构化集成方法显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 人类生活在物理4D世界中，几何结构和语义内容随时间演化构成动态现实。当前多模态大语言模型在静态视觉理解方面表现出色，但缺乏对动态场景中时空动态的感知、跟踪和推理能力评估。需要系统评估模型的时空推理和局部动态感知能力。

Method: 从多样化的真实世界和合成视频数据集中构建大规模基准测试Dyn-Bench，通过多阶段过滤从大量2D和4D数据源中筛选高质量动态场景，包含1k视频、7k视觉问答对和3k动态物体定位对。采用结构化集成方法，包括掩码引导融合和时空文本认知图来增强模型性能。

Result: 现有模型无法同时在时空推理和动态物体定位上保持强大性能，经常产生对运动和交互的不一致解释。传统提示策略（如思维链或基于标题的提示）改进有限，而结构化集成方法显著增强了多模态大语言模型在物理4D世界中的动态感知和时空推理能力。

Conclusion: Dyn-Bench为评估多模态大语言模型的动态思维能力提供了系统基准，揭示了现有模型在动态场景理解中的局限性，并展示了结构化集成方法在提升时空动态感知方面的有效性，为未来动态4D世界理解研究奠定了基础。
...
TL;DR: SLICE是一种新的扩散模型水印方法，通过将图像语义分解为四个语义因子（主体、环境、动作、细节），并将它们精确锚定到初始高斯噪声的不同区域，实现了细粒度的语义感知水印，能够检测和定位语义篡改。


<details>
  <summary>Details</summary>
Motivation: 现有基于初始噪声的水印方法容易受到反转和再生攻击，而最近的语义感知水印方法虽然提高了鲁棒性，但由于依赖单一的全局语义绑定，容易受到局部但全局一致的语义编辑攻击。需要一种更细粒度、更可靠的语义感知水印方案。

Method: 提出SLICE框架，将图像语义解耦为四个语义因子：主体、环境、动作和细节，并将这些因子精确锚定到初始高斯噪声的不同区域。通过这种细粒度的语义绑定，实现高级水印验证，能够检测和定位语义篡改。

Result: SLICE在对抗先进的语义引导再生攻击方面显著优于现有基线方法，大幅降低了攻击成功率，同时保持了图像质量和语义保真度。该方法提供了统计上可保证的误接受率，并实现了鲁棒可靠的篡改定位。

Conclusion: SLICE提供了一种实用的、无需训练的溯源解决方案，既具有细粒度的诊断能力，又对现实中的对抗性操作具有鲁棒性，为扩散模型图像溯源提供了更可靠的语义感知水印方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有闭集物体检测器无法识别演示中的新物体，而开集检测器需要昂贵的人工提示工程，需要更高效的解决方案

Method: 开发自监督系统，利用人类演示本身作为监督信号自动创建数据集，训练针对特定物体的定制化检测器，完全绕过语言描述

Result: 系统显著优于最先进的检测和识别方法，提高了机器人任务完成率

Conclusion: "Show, Don't Tell"范式通过演示而非语言描述训练定制检测器，为机器人快速识别新物体提供了有效解决方案
...
TL;DR: FC-Track是一个轻量级的多目标跟踪后关联校正框架，专门解决物体重叠导致的ID切换问题，通过IoA过滤策略和局部外观相似性比较，在不依赖全局优化或重识别的情况下提升跟踪可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有的在线多目标跟踪方法在复杂动态环境中容易因频繁遮挡和物体重叠而导致身份切换，错误的关联会随时间传播并降低跟踪可靠性，需要一种轻量级的解决方案来专门处理重叠引起的误匹配问题。

Method: 提出基于交集面积比(IoA)的过滤策略来抑制高重叠条件下的不可靠外观更新，并通过重叠轨迹对内的外观相似性比较来局部校正检测到轨迹的误匹配，防止短期误匹配传播为长期身份切换。
...
Conclusion: FC-Track框架有效缓解了重叠引起的身份切换问题，无需全局优化或重识别即可在线运行，适合实时机器人应用，在MOT20基准上保持了最先进的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于透视图像训练的基础模型在360°全景图像上的性能会显著下降，而可提示的实例分割在具身系统和AR系统中被广泛采用，因此需要专门针对高分辨率全景图像的分割模型。
...
Result: SAP在真实世界360°图像上表现出良好的泛化能力，在真实世界4K全景基准测试中，相比不同尺寸的原始SAM2实现了+17.2的零样本mIoU提升。

Conclusion: SAP通过轨迹对齐的范式有效解决了全景分割问题，为高分辨率全景图像的实例分割提供了强大的基础模型，显著提升了在真实世界全景图像上的性能。
...
TL;DR: 本文提出HiFICL方法，通过虚拟键值对、低秩分解和端到端训练目标，更准确地建模多模态大模型的上下文学习机制，相比现有近似方法在多个基准测试中表现更优。
...
Conclusion: HiFICL通过更准确地建模上下文学习机制，提供了一种有效的上下文感知参数高效微调方法，在多模态任务中表现出色。
...
TL;DR: TerraFlow是一种用于地球观测的多模态、多时序学习方法，通过时序训练目标实现跨空间、时间和模态的序列感知学习，在GEO-Bench-2基准测试的所有时序任务中优于现有基础模型，并在自然灾害风险图预测方面表现突出。


<details>
  <summary>Details</summary>
Motivation: 地球观测数据通常具有多模态、多时序和变长输入的特点，现有基础模型在处理这些复杂数据时存在局限性，特别是在自然灾害风险预测等关键任务上经常失效。

Method: 提出TerraFlow方法，基于时序训练目标实现跨空间、时间和模态的序列感知学习，能够处理真实世界地球观测数据中常见的变长输入。

Result: 在GEO-Bench-2基准测试的所有时序任务中优于现有最先进的基础模型，在自然灾害风险图预测任务上表现尤为突出，F1分数提升高达50%，Brier分数提升24%。
...
TL;DR: 本文提出SAVA-X框架，解决从第三人称（exo）演示评估第一人称（ego）模仿的错误检测问题，处理跨视角域偏移、时间不对齐和冗余挑战。


<details>
  <summary>Details</summary>
Motivation: 现有错误检测方法主要针对单视角设置，无法处理使用第三人称演示评估第一人称模仿的实际场景。这种跨视角设置引入了域偏移、时间不对齐和冗余等挑战。
...
Conclusion: SAVA-X有效解决了Ego→Exo模仿错误检测问题，克服了跨视角域偏移、时间不对齐和冗余等挑战，在工业训练、医疗和装配质量控制等领域具有应用价值。
...
Method: Catalyst4D框架包含两个核心组件：1）锚点运动引导（AMG）- 从原始和编辑后的高斯分布中构建结构稳定且空间代表性的锚点集合，通过最优传输建立对应关系，实现一致的变形传播；2）颜色不确定性引导的外观优化（CUAR）- 通过估计每个高斯的颜色不确定性，选择性优化容易产生遮挡伪影的区域，保持时间外观一致性。

Result: 大量实验表明，Catalyst4D实现了时间稳定的高保真动态场景编辑，在视觉质量和运动一致性方面优于现有方法。

Conclusion: Catalyst4D通过锚点运动引导和颜色不确定性外观优化的创新组合，成功解决了动态4D高斯场景编辑中的时空一致性问题，为高质量动态场景编辑提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLA架构中，预训练的视觉语言模型（VLM）通常针对语义抽象优化，基于静态视觉观察，会减弱细粒度几何线索且缺乏显式时间证据，影响动作专家的性能

Method: 提出Plug-in Visual Injection (PVI)模块，这是一种轻量级、编码器无关的模块，通过零初始化残差路径连接到预训练动作专家，注入辅助视觉表示，仅需单阶段微调即可保持预训练行为

Result: PVI相比基础策略和多种竞争性注入策略获得一致性能提升，控制研究表明时间视频特征（V-JEPA2）优于静态图像特征（DINOv2），在需要状态跟踪和协调的多阶段任务上提升最大；真实机器人实验验证了PVI在长视野双手机器人布料折叠任务中的实用性

Conclusion: PVI是一种有效的视觉特征注入方法，能够提升VLA架构在语言条件操作任务中的性能，特别是时间视频特征在多阶段任务中表现更优，具有实际应用价值
...
TL;DR: 提出了一个包含10种基本手术动作、6个外科专科、超过11,000个视频片段的最大BSA数据集，并基于此开发了能够跨专科识别基本手术动作的基础模型，展示了在手术技能评估和动作规划等下游应用中的潜力。
...
Method: 创建了包含10种基本手术动作、涵盖6个外科专科、超过11,000个视频片段的最大BSA数据集。基于该数据集开发了一个能够进行通用基本动作识别的基础模型，并在不同手术类型和身体部位的验证数据集上测试了跨专科性能。

Result: 模型在不同手术类型和身体部位的验证数据集上表现出稳健的跨专科性能。通过领域特定知识在前列腺切除术中进行手术技能评估，以及使用大视觉语言模型在胆囊切除术和肾切除术中进行动作规划，展示了基础模型的下游应用潜力。多国外科医生对语言模型输出的动作规划解释文本进行了临床相关性评估。
...
Method: 提出了多实体推理遥感定位基准数据集ME-RSRG，并设计了实体感知推理框架EAR。该框架基于视觉语言基础模型生成结构化推理轨迹和主客体定位输出，采用监督微调进行冷启动初始化，并通过实体感知奖励驱动的组相对策略优化进一步优化。

Result: 在ME-RSRG数据集上的大量实验证明了多实体推理的挑战性，并验证了所提出的EAR框架的有效性。

Conclusion: 该研究成功将推理范式扩展到遥感视觉定位领域，通过ME-RSRG数据集和EAR框架解决了多实体推理定位问题，为遥感图像理解提供了新的研究方向。
...
TL;DR: Cheers是一个统一的多模态模型，通过将补丁级细节与语义表示解耦，在单个模型中同时实现视觉理解和生成，实现了4倍令牌压缩和20%的训练成本。
...
Result: 在流行基准测试中，Cheers在视觉理解和生成方面都匹配或超越了先进的UMMs。实现了4倍令牌压缩，使高分辨率图像编码和生成更高效。在GenEval和MMBench上优于Tar-1.5B，仅需20%的训练成本。
...
TL;DR: 本文针对3D高斯泼溅(3DGS)的资源定向攻击提出频谱防御方法，通过3D频率滤波器和2D频谱正则化抑制恶意扰动导致的高斯过度增长。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅(3DGS)虽然能提供高质量渲染，但其高斯表示暴露了新的攻击面——资源定向攻击。攻击者通过毒化训练图像，诱导高斯过度增长导致资源耗尽。现有的空间域防御方法(如平滑、阈值、剪枝)只处理可见结构，忽略了恶意扰动如何扭曲训练数据的底层频谱行为。

Method: 提出高斯场和图像场的频谱防御：1) 设计3D频率滤波器，选择性剪枝表现出异常高频的高斯；2) 开发2D频谱正则化，在渲染上区分自然各向同性频率，同时惩罚各向异性角能量以约束噪声模式。

Result: 实验表明，该防御方法能构建鲁棒、准确且安全的3DGS：在攻击下抑制过度增长达5.92倍，减少内存达3.66倍，提升速度达4.34倍。

Conclusion: 通过频谱防御方法，能够有效应对3DGS的资源定向攻击，在保持场景保真度的同时防止高斯过度增长，提升系统的安全性和效率。
...
TL;DR: 本文提出R-Adapt框架，通过冻结预训练权重并仅在浅层引入最小化适配，在视觉语言模型中实现了对抗鲁棒性与干净数据准确性的优异平衡。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型中的对抗鲁棒性通常以牺牲干净数据准确性为代价，存在长期存在的挑战性权衡。本文旨在探究VLMs鲁棒性的本质机制，以解决这一权衡问题。
...
Conclusion: 对抗鲁棒性在VLMs中主要分布在浅层，而非均匀分布。R-Adapt通过冻结预训练权重并仅在浅层进行最小化适配，实现了对抗鲁棒性与干净数据准确性的优异平衡，为增强标准模型的鲁棒性提供了灵活途径。
...
TL;DR: OARS是一个在线对齐框架，使用MLLM奖励模型COMPASS评估超分辨率图像质量，通过渐进式在线对齐实现感知质量提升同时保持保真度。


<details>
  <summary>Details</summary>
Motivation: 生成式真实世界图像超分辨率模型与人类视觉偏好对齐具有挑战性，因为存在感知-保真度权衡和多样未知退化。现有方法依赖离线偏好优化和静态指标聚合，通常难以解释且在强条件化下容易产生伪多样性。

Method: 提出OARS框架，基于COMPASS奖励模型评估LR到SR的转换，联合建模保真度保持和感知增益。COMPASS通过三阶段感知标注流程训练。OARS执行渐进式在线对齐：从冷启动流匹配到全参考，再到无参考RL，通过浅层LoRA优化进行策略探索。

Result: 在Real-ISR基准测试中实现最先进性能，实验和用户研究表明在保持保真度的同时获得一致的感知改进。

Conclusion: OARS框架通过在线对齐和基于MLLM的奖励模型，有效解决了真实世界图像超分辨率中的感知-保真度权衡问题，实现了与人类视觉偏好的更好对齐。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成模型在处理复杂场景时，难以忠实地组合多个对象并保持其属性，需要更有效的组合生成方法。

Method: 提出四智能体框架：解释器自适应选择直接生成或布局感知多智能体流程；规划器采用分治策略逐步规划布局；检查器验证空间一致性和属性对齐；绘制器逐步合成图像。

Result: 在GenEval和DPG-Bench基准测试中，coDrawAgents显著提高了文本-图像对齐、空间准确性和属性绑定，优于现有方法。

Conclusion: coDrawAgents通过多智能体协作框架有效解决了文本到图像生成中的组合挑战，为复杂场景生成提供了新思路。
...
TL;DR: 本文提出无人机场景变化描述任务(UAV-SCC)，旨在从移动视角的动态航拍图像中生成语义变化的自然语言描述，并提出了HDC-CL方法来解决视角变化带来的挑战。


<details>
  <summary>Details</summary>
Motivation: 传统变化描述主要处理固定视角下的图像对差异，而无人机场景变化描述需要同时处理时间和空间变化，特别是由相机旋转引起的视角变化，这导致图像对只有部分重叠场景内容，增加了理解难度。

Method: 提出分层双变协同学习(HDC-CL)方法，包括动态自适应布局变换器(DALT)来建模图像对的不同空间布局，以及分层跨模态方向一致性校准(HCM-OCC)来增强模型对视角偏移方向的敏感性。

Result: 构建了新的UAV-SCC基准数据集，并通过大量实验证明所提方法在该任务上达到了最先进的性能。

Conclusion: 本文提出了无人机场景变化描述这一新任务，开发了有效的HDC-CL方法，并创建了相应的数据集，为动态航拍图像理解提供了新的研究方向。
...
Method: 提出多模态融合方法：1) 使用VideoMAE模型捕捉场景动态；2) 通过情感帧级嵌入和统计池化编码面部信息；3) 使用EmotionWav2Vec2.0提取声学表征，Mamba时序编码器处理；4) 微调transformer文本模型建模语言线索；5) 采用多模态融合模型（包括原型增强变体）整合各模态嵌入。

Result: 在BAH语料库上，最佳单模态配置平均MF1为70.02%，最佳多模态融合模型达到83.25%。最终测试性能最高为71.43%，由五个原型增强融合模型的集成获得。

Conclusion: 结果表明互补的多模态线索和鲁棒的融合策略对于犹豫/矛盾识别至关重要，多模态融合显著优于所有单模态基线。
...
<details>
  <summary>Details</summary>
Motivation: 磨料瓣轮因其灵活性常用于复杂自由曲面的精加工，但这种灵活性导致复杂的磨损模式（如凹凸瓣轮轮廓或瓣轮撕裂），这些磨损会影响研磨结果。目前缺乏自动化监测磨损状况的方法。
...
Conclusion: 提出的分层方法为自适应过程控制和自动化瓣轮研磨中的磨损考虑提供了基础，实现了对瓣轮磨损状况的自动化监测。
...
TL;DR: CompoSIA是一个解耦交通要素的驾驶视频模拟器，能够独立控制场景结构、物体身份和自车动作，用于合成安全关键边缘案例。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶面临"长尾"安全关键边缘案例的挑战，这些案例通常来自常见交通要素的不寻常组合。现有可控生成模型提供不完整或纠缠的指导，无法独立操纵场景结构、物体身份和自车动作。

Method: 提出CompoSIA，一个解耦交通要素的驾驶视频模拟器。采用噪声级身份注入实现姿态无关的身份生成，以及分层双分支动作控制机制提升动作可控性。

Result: 在身份编辑方面FVD提升17%，动作控制方面旋转和翻译误差分别降低30%和47%。下游压力测试显示碰撞率平均增加173%。

Conclusion: CompoSIA通过解耦控制实现了对抗性场景合成，能够将安全要素系统性地组合成危险配置，这是纠缠生成器无法实现的，为自动驾驶安全测试提供了有效工具。
...
TL;DR: TRACE是一个基于扩散模型的结构感知字符编码框架，通过利用字符结构的稳定性来嵌入数据，相比现有方法在抗噪性和性能上有显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有字符编码方法通常依赖边缘特征或预定义码本，这些方法对噪声干扰敏感。字符结构具有稳定性和统一表示的特点，能提供更好的抗噪性，因此需要开发一种能充分利用字符结构优势的编码框架。
...
Result: 实验表明TRACE在性能上显著优于现有方法，PSNR提升超过5dB，跨媒体传输后的提取准确率提高5%。框架在多种语言和字体上具有广泛泛化能力。

Conclusion: TRACE通过利用字符结构的固有稳定性，实现了高效、抗噪的字符编码，特别适用于实际文档安全应用，在性能、准确性和泛化能力方面均表现出色。
...
TL;DR: 视频癫痫发作预测：利用短时发作前视频（3-10秒）预测未来5秒内是否会发生癫痫发作，通过跨物种迁移学习框架解决人类视频数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 现有癫痫预测方法主要依赖EEG等神经信号，需要专业设备且难以长期部署。视频数据提供非侵入性替代方案，但现有研究主要关注发作后检测，发作预测研究不足。需要开发基于视频的癫痫发作预测方法。

Method: 提出视频癫痫发作预测任务，使用短时发作前视频段（3-10秒）预测未来5秒内发作。为解决人类癫痫视频标注数据稀缺问题，提出跨物种迁移学习框架，利用大规模啮齿动物视频数据进行辅助预训练，捕捉跨物种通用的发作相关行为动态。

Result: 实验结果显示，在严格仅使用视频数据的设置下，该方法达到超过70%的预测准确率，并优于现有基线方法。

Conclusion: 研究证明了跨物种学习在构建非侵入性、可扩展的癫痫早期预警系统方面的潜力，为基于视频的癫痫发作预测提供了新思路。
...
TL;DR: 提出一种在线强化学习变体，通过采样配对轨迹并将流速度拉向更有利图像方向来减少模型更新方差，将整个采样过程视为单一动作而非每一步单独处理。
...
Method: 提出在线RL变体，通过采样配对轨迹并将流速度拉向更有利图像方向来减少方差。将整个采样过程视为单一动作而非每一步单独处理。使用高质量视觉语言模型和现成质量指标作为奖励。

Result: 方法收敛更快，在输出质量和提示对齐方面优于先前方法。使用广泛指标集进行评估验证了效果。

Conclusion: 提出的在线RL变体通过将整个采样过程视为单一动作并减少方差，在扩散模型后训练中实现了更快的收敛和更好的输出质量与提示对齐。
...
TL;DR: SG-NLF是一个无需姿态估计的LiDAR NeRF框架，通过融合光谱信息和几何一致性来解决LiDAR数据稀疏、无纹理导致的几何空洞和表面不连续问题。
...
Conclusion: SG-NLF为LiDAR视角合成提供了新的视角，通过光谱信息与几何一致性的融合，有效解决了姿态依赖和LiDAR数据特性带来的挑战。
...
TL;DR: 提出了一种基于语义锚点的结构保持机器遗忘框架，通过维护知识结构来平衡数据删除与保留，在多个视觉任务上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘方法往往忽视保留数据之间的语义关系，导致模型出现渐进式结构崩溃，破坏了删除与保留之间的平衡。需要一种能够保持知识结构的方法来更好地实现机器遗忘。
...
Conclusion: 通过语义锚点保持知识结构是机器遗忘的关键，提出的结构保持框架能够有效防止模型结构崩溃，在多个视觉任务上显著提升性能，为机器遗忘提供了新的解决方案。
...
TL;DR: 本文提出VIRD方法，通过双轴变换构建视角不变表示，解决地面与卫星视图间的视角差异问题，显著提升跨视图姿态估计精度。
...
Method: 提出VIRD方法：首先对卫星视图应用极坐标变换建立水平对应关系，然后使用上下文增强的位置注意力处理地面和极坐标变换后的卫星特征，解决垂直错位问题，并引入视图重建损失进一步增强视角不变性。

Result: 在KITTI和VIGOR数据集上的实验表明，VIRD在无方向先验条件下优于现有方法：KITTI数据集上位置和方向中值误差分别降低50.7%和76.5%，VIGOR数据集上分别降低18.0%和46.8%。

Conclusion: VIRD通过双轴变换和视图重建损失有效缓解了地面与卫星视图间的视角差异，显著提升了跨视图姿态估计的精度，为自动驾驶和机器人定位提供了更可靠的解决方案。
...
TL;DR: 提出IFDL-VLM方法，利用视觉语言模型辅助图像伪造检测与定位，通过位置掩码先验缓解模型偏见，在9个基准测试中实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 随着AIGC技术的快速发展，图像伪造变得日益容易，给图像伪造检测与定位带来巨大挑战。传统视觉语言模型在IFDL任务中存在偏见，倾向于语义合理性而非真实性，甚至对检测性能产生负面影响。

Method: 提出IFDL-VLM新流程，利用位置掩码显式编码伪造概念作为额外先验，帮助视觉语言模型缓解训练优化难度，增强检测和定位结果的可解释性。

Result: 在9个流行基准测试上进行实验，评估域内和跨数据集泛化性能。实验结果表明，该方法在检测、定位和可解释性方面均取得新的最先进性能。

Conclusion: 通过利用位置掩码先验来缓解视觉语言模型的偏见，提出的IFDL-VLM方法能有效提升图像伪造检测与定位的性能和可解释性，为AIGC时代的图像真实性验证提供有力工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有功能映射方法在非等距变形和拓扑噪声下存在模糊性，几何描述符无法单独解决这些问题，且谱基投影到密集点对应时存在空间不一致性

Method: 设计语义引导局部交叉注意力模块，将视觉基础模型的语义特征集成到几何描述符中，同时保持局部结构连续性；引入基于条件流匹配的正则化目标，监督时变速度场以鼓励恢复对应关系的空间平滑性

Result: 在多个基准测试中，SGMatch在近等距设置下达到竞争性性能，在非等距变形和拓扑噪声下实现一致改进

Conclusion: SGMatch通过语义引导和流匹配正则化有效解决了非刚性形状匹配中的模糊性和空间不一致问题，特别是在非等距变形和拓扑噪声场景下表现优异
...
TL;DR: 该论文提出了一种名为CleanSight的训练免费、即插即用防御方法，用于检测和净化大型视觉语言模型中的后门攻击，通过分析跨模态注意力重新分配机制来实现。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在微调时容易受到后门攻击，现有防御方法通常需要重新训练被感染的参数，计算成本高且会降低模型性能。作者发现后门攻击通过异常跨模态注意力重新分配机制（注意力窃取）影响预测，而非通过低级视觉模式。

Method: 提出CleanSight防御方法：1）基于选定跨模态融合层中的视觉-文本注意力相对比例检测中毒输入；2）通过选择性修剪可疑的高注意力视觉标记来净化输入，从而中和后门激活。该方法完全在测试时运行，无需训练。

Result: 大量实验表明，CleanSight在不同数据集和后门攻击类型上显著优于现有的基于像素的净化防御方法，同时在干净和中毒样本上都保持了模型的实用性。

Conclusion: 通过揭示后门攻击的注意力窃取机制，提出了一个高效、训练免费的防御框架CleanSight，为大型视觉语言模型的后门防御提供了新思路，在保持模型性能的同时有效抵御攻击。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型（VLMs）会继承训练数据中的社会偏见并传播到下游应用中，现有去偏方法大多缺乏理论保证，无法确保模型效用的保持。

Method: 在跨模态空间中设计闭式解的去偏方法，无需训练和标注数据，可同时处理视觉和文本模态的偏见，适用于多种下游任务。

Result: 在零样本图像分类、文本到图像检索、文本到图像生成等任务中，该方法在群体公平性和交叉公平性指标上优于现有方法，同时保持了任务性能。

Conclusion: 该方法为视觉语言模型提供了一种理论保证的、高效的跨模态去偏解决方案，实现了公平性与效用之间的帕累托最优平衡。
...
TL;DR: SAW是一个基于扩散模型的手术动作世界模型，通过语言提示、参考场景、组织可操作性掩码和工具轨迹四个轻量级信号控制，生成逼真的手术动作视频，用于解决手术AI数据稀缺和模拟器真实感不足的问题。


<details>
  <summary>Details</summary>
Motivation: 当前手术视频生成方法需要昂贵标注或复杂中间表示作为条件信号，限制了可扩展性；现有方法在复杂腹腔镜场景中时间一致性有限，真实感不足。需要一种能够生成逼真手术动作视频、精确控制工具-组织交互的世界模型，以解决手术AI数据稀缺、罕见事件合成和模拟器真实感差距等问题。

Method: 提出Surgical Action World (SAW)模型，基于条件视频扩散方法，将视频到视频扩散重新表述为轨迹条件的手术动作合成。模型使用四个轻量级条件信号：编码工具-动作上下文的语言提示、参考手术场景、组织可操作性掩码和2D工具尖端轨迹。在12,044个腹腔镜视频片段的自定义数据集上微调骨干扩散模型，采用深度一致性损失增强几何合理性，无需推理时的深度信息。

Result: SAW在保留测试数据上实现了最先进的时间一致性（CD-FVD: 199.19 vs. 546.82）和强大的视觉质量。在下游应用方面：(a) 手术AI：用SAW生成的视频增强罕见动作，显著提高了动作识别性能（夹闭F1分数：20.93%到43.14%；切割：0.00%到8.33%）；(b) 手术模拟：从模拟器导出的轨迹点渲染工具-组织交互视频，实现了视觉逼真的模拟引擎。

Conclusion: SAW通过轻量级条件信号实现了高质量、时间一致的手术动作视频生成，为手术AI数据增强和手术模拟器真实感提升提供了有效解决方案，是迈向手术动作世界建模的重要一步。
...
TL;DR: SortScrews数据集：一个用于螺丝分类的公开数据集，包含560张RGB图像，覆盖6种螺丝类型和背景类，为工业自动化中的螺丝识别提供基准。


<details>
  <summary>Details</summary>
Motivation: 工业自动化、机器人和库存管理中螺丝类型的自动识别很重要，但目前公开的螺丝分类数据集稀缺，特别是在自动化分拣系统中常见的受控单物体场景。

Method: 创建SortScrews数据集，包含560张512×512分辨率的RGB图像，覆盖6种螺丝类型和背景类；使用标准化采集设置，包含光照和相机视角的轻微变化；提供可重复使用的数据收集脚本；使用EfficientNet-B0和ResNet-18进行迁移学习基准测试。

Result: 尽管数据集规模有限，轻量级模型仍能实现强大的分类准确率，表明受控采集条件即使在相对较小的数据集上也能实现有效学习。

Conclusion: SortScrews数据集填补了螺丝分类数据集的空白，为工业自动化应用提供了有价值的资源；受控采集条件使小数据集也能实现有效学习；公开的数据集、收集管道和基准训练代码促进了可重复研究。
...
TL;DR: MOCR提出多模态OCR范式，将文本和图形统一解析为结构化表示，超越传统OCR仅识别文本的局限，在文档解析和结构化图形解析任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统OCR系统主要关注文本识别，将图形区域作为裁剪像素处理，无法捕捉文本与视觉元素之间的语义关系，限制了文档的完整解析和重建能力。

Method: 提出dots.mocr方法，将图表、图示、表格、图标等视觉元素作为一级解析目标，构建从PDF、网页渲染和原生SVG资产的数据引擎，通过分阶段预训练和监督微调训练3B参数模型。
...
Conclusion: MOCR范式为构建大规模图像到代码语料库进行多模态预训练提供了可扩展路径，实现了文本和图形的统一解析，提升了文档的语义理解和重建能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型的空间认知评估在范式和覆盖范围上都有限制，阻碍了模型的快速迭代开发。需要一种更全面、更贴近实际部署的评估方法。

Method: 提出ESPIRE基准，包含模拟世界和机器人任务。将任务分解为定位和执行两个生成性问题，与依赖干扰项的判别式评估形成对比。在指令和环境层面系统设计，确保广泛覆盖空间推理场景。

Result: 使用ESPIRE对前沿视觉语言模型进行诊断，深入分析它们的空间推理行为。

Conclusion: ESPIRE提供了一个物理基础的评估框架，能够更全面地评估视觉语言模型的具身空间推理能力，促进模型开发。
...
TL;DR: 该论文提出Topo-R1框架，用于无标注的拓扑异常检测，通过两阶段训练使视觉语言模型具备拓扑感知能力，在多个领域基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有拓扑保持方法依赖特定领域的标注数据，成本高且难以跨领域迁移。当部署到新领域时，需要无监督检测拓扑异常的方法。视觉语言模型虽然适合此类结构化视觉推理任务，但现有模型缺乏细粒度拓扑感知能力。

Method: 1) 开发自动化数据生成流程，合成多样化的拓扑异常并构建多领域基准；2) 提出Topo-R1框架，通过监督微调和基于GRPO的强化学习两阶段训练；3) 设计拓扑感知复合奖励函数，包括类型感知匈牙利匹配、空间定位评分和中心线Dice奖励。

Result: Topo-R1在多个评估协议中一致优于通用视觉语言模型和监督基线，建立了无标注拓扑质量评估的新范式。实验表明该方法能够有效检测稀疏连接错误并保持结构保真度。

Conclusion: 该研究成功解决了无监督拓扑异常检测问题，通过数据合成和两阶段训练使视觉语言模型具备拓扑感知能力，为血管、神经纤维等管状结构的拓扑质量评估提供了有效解决方案。
...
TL;DR: 提出了一种用于野外条件下连续情感识别的多模态方法，结合面部、行为和音频三种模态，在Aff-Wild2数据集上取得了0.658的CCC分数。


<details>
  <summary>Details</summary>
Motivation: 野外条件下的连续情感识别面临巨大挑战，包括外观变化、头部姿态、光照、遮挡和个体表达差异等问题，需要开发鲁棒的多模态方法来应对这些复杂情况。

Method: 采用三种互补模态：1) 面部模态使用GRADA帧级嵌入和Transformer时序回归；2) 行为模态使用Qwen3-VL-4B-Instruct提取视频片段信息，Mamba建模时序动态；3) 音频模态使用WavLM-Large和注意力统计池化，包含跨模态过滤阶段。探索两种融合策略：定向跨模态专家混合融合和可靠性感知视听融合。

Result: 在Aff-Wild2开发集上，提出的多模态融合策略实现了0.658的Concordance Correlation Coefficient (CCC)，遵循ABAW挑战协议进行评估。

Conclusion: 该方法通过有效结合面部、行为和音频模态，并采用创新的融合策略，在野外连续情感识别任务中取得了有竞争力的性能，证明了多模态方法在处理复杂野外条件下的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 虚拟试穿系统在时尚电商等应用中日益重要，但缺乏可靠的评估方法。真实场景中通常没有目标服装的参考图像，而现有的分布级指标无法反映单个生成图像的感知质量。

Method: 提出VTON-IQA框架，包含：1) 构建VTON-QBench基准数据集（62,688张试穿图像，431,800个人工标注）；2) 引入交错交叉注意力模块，在标准Transformer块的自注意力和MLP之间插入交叉注意力层，显式建模服装保真度和人物细节保留的交互。

Result: VTON-IQA实现了可靠的人类对齐图像级质量预测。使用该框架对14个代表性VTON模型进行了全面基准评估，证明了其有效性。

Conclusion: VTON-IQA是首个无需参考图像、基于人类感知的虚拟试穿质量评估框架，解决了现有评估方法的局限性，为VTON系统的可靠评估提供了有效工具。
...
TL;DR: 本文提出了两种互补方法：RAPTA通过区域感知提示增强减少训练图像记忆，ADMCD通过注意力驱动多模态复制检测识别抄袭，在保持图像质量的同时降低版权风险。
...
Method: 提出了两种互补方法：1) RAPTA：使用对象检测器找到显著区域，将其转换为语义基础的提示变体，在训练期间随机采样以增加多样性，同时保持语义对齐；2) ADMCD：通过轻量级transformer聚合局部补丁、全局语义和纹理线索，生成融合表示，并应用简单的阈值决策规则来检测复制，无需大型标注数据集训练。

Result: 实验表明，RAPTA在保持高质量合成的同时减少了过拟合，ADMCD能够可靠地检测复制，性能优于单模态指标。

Conclusion: RAPTA和ADMCD为文本到图像扩散模型提供了有效的解决方案，既能减少训练数据记忆带来的版权风险，又能保持生成质量，同时提供可靠的复制检测能力。
...
<details>
  <summary>Details</summary>
Motivation: 实时屋顶风速分布对于无人机安全运行、城市空中交通系统、风控系统和屋顶利用至关重要，但屋顶气流具有强非线性、分离和交叉方向变异性，使得从稀疏传感器重建流场变得困难。

Method: 使用粒子图像测速仪（PIV）获取的风洞实验数据，开发学习观察框架，比较克里金插值与三种深度学习模型（UNet、ViTAE、CWGAN）。评估两种训练策略（单风向训练SDT和混合风向训练MDT），传感器密度从5到30，测试传感器位置扰动（±1网格）下的鲁棒性，并通过带QR分解的适当正交分解优化传感器布局。

Result: 深度学习方法能有效从稀疏传感器数据重建屋顶风场。相比克里金插值，深度学习模型将SSIM提高达32.7%，FAC2提高24.2%，NMSE降低27.8%。混合风向训练进一步改善性能，SSIM增益达173.7%，FAC2增益16.7%，MG增益98.3%。QR优化在传感器扰动下将鲁棒性提高达27.8%，但存在指标依赖的权衡。

Conclusion: 传感器配置、优化和训练策略应联合考虑以确保可靠部署。基于实验而非模拟数据的训练为不同场景下的方法选择和传感器布局提供了实用指导。
...
<details>
  <summary>Details</summary>
Motivation: 文本引导的3D运动编辑在单人场景中已有成功应用，但在多人场景中研究较少，主要受限于配对数据不足和人际交互的复杂性。

Method: 提出InterEdit3D数据集和TMME基准，并开发InterEdit模型——一个同步的无分类器条件扩散模型，包含语义感知计划token对齐和交互感知频率token对齐策略。
...
TL;DR: V-Bridge框架利用预训练视频生成模型作为通用视觉学习器，通过极少量数据（仅1000个样本）激活其内在的图像修复能力，实现多任务统一模型，挑战了生成建模与低级视觉任务的传统界限。


<details>
  <summary>Details</summary>
Motivation: 大规模视频生成模型内部蕴含丰富的视觉世界结构、语义和动态先验知识，但这些模型作为通用视觉学习器的潜力尚未被充分挖掘。传统图像修复方法通常需要大量任务特定数据和专门架构，而视频模型的内在修复能力未被有效利用。

Method: 将图像修复重新定义为渐进式生成过程而非静态回归问题，利用视频模型模拟从退化输入到高保真输出的逐步细化过程。通过V-Bridge框架，仅用1000个多任务训练样本（不到现有方法的2%）激活预训练视频模型的修复能力。

Result: 仅用极少量数据，预训练视频模型就能实现具有竞争力的图像修复性能，单个模型可处理多个修复任务，性能与专门为此目的设计的专用架构相当，展现了视频生成模型隐含的强大可迁移修复先验。

Conclusion: 视频生成模型隐含学习到强大且可迁移的修复先验知识，这些知识可通过极少数据被激活，挑战了生成建模与低级视觉任务的传统界限，为视觉任务的基础模型设计开辟了新范式。
...
TL;DR: VAEX-BENCH是一个评估多模态大语言模型抽象时空推理能力的基准，包含五个抽象推理任务及其对应的提取式任务，通过可控的合成第一人称视频数据集进行测试。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解基准主要关注提取式推理，即答案可以直接从时空事件中明确找到。然而，对于需要整合时间观察、结合分散线索、推断隐式空间和上下文结构的抽象时空推理能力，目前缺乏系统评估。

Method: 1. 形式化视频的抽象时空推理概念；2. 引入结构化评估分类法，系统针对其核心维度；3. 构建可控、场景驱动的合成第一人称视频数据集，专门评估抽象时空推理能力，涵盖物体级、房间级和楼层平面图级场景；4. 基于此框架提出VAEX-BENCH基准，包含五个抽象推理任务及其提取式对应任务。

Result: 通过广泛实验比较了最先进的多模态大语言模型在提取式和抽象式设置下的性能，暴露了它们在抽象任务上的局限性，并对底层瓶颈进行了细粒度分析。

Conclusion: VAEX-BENCH基准填补了抽象时空推理评估的空白，揭示了当前多模态大语言模型在抽象推理能力上的不足，为未来研究提供了系统评估框架。数据集即将发布。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于学习的制造性预测方法面临两大挑战：1）制造性定义不一致导致学习目标混乱；2）缺乏合适的数据集。现有数据集要么只包含可制造零件，要么只关注简单几何形状和减材工艺。

Method: 1）提出了制造性度量的分类体系，从配置依赖性和测量类型两个维度进行划分；2）开发了BenDFM数据集，包含20,000个通过工艺感知弯曲模拟生成的零件，提供折叠和展开几何形状以及多种制造性标签。

Result: 在BenDFM数据集上对两种最先进的3D学习架构进行基准测试，发现基于图的表示方法（能捕捉零件表面关系）获得更好的准确性，但预测依赖于特定制造设置的度量仍然更具挑战性。

Conclusion: BenDFM数据集填补了制造性评估领域的空白，为系统研究基于学习的DFM挑战提供了基础。基于图的表示方法在制造性预测方面表现更好，但配置依赖的度量预测仍需进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 挑战当前医学影像处理中普遍采用的离散网格深度学习范式，解决固定分辨率限制和离散化敏感性问题，实现更灵活、鲁棒的医学影像分析。

Method: 将离散医学信号嵌入共享的隐式神经表示，学习神经算子来映射其潜在调制，实现分辨率无关的函数到函数变换，而非在固定像素/体素网格上操作。

Result: 在多个2D和3D下游任务（分割、形状补全、图像翻译、图像合成）上达到竞争性性能，在原生分辨率下表现良好，对未见离散化具有强鲁棒性，并经验性满足神经算子的关键理论性质。

Conclusion: NOIR框架成功将医学影像任务重构为连续函数空间间的算子学习，为医学影像分析提供了分辨率无关、鲁棒性强的新范式，具有重要的理论和实践意义。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频语言模型很少显式表示相机运动这一基本几何信号，且在精细运动基元识别上经常失败。相机运动是塑造视觉感知和电影风格的关键因素，这一能力缺失限制了模型对视频内容的理解。

Method: 1. 创建大规模合成数据集CameraMotionDataset，包含显式相机控制；2. 将相机运动建模为约束感知的多标签识别问题；3. 构建VQA基准CameraMotionVQA；4. 提出轻量级、模型无关的流水线：从3D基础模型提取几何相机线索，使用时序分类器预测约束运动基元，通过结构化提示注入下游VideoLLM推理。

Result: 实验发现现有VideoLLMs在识别相机运动基元时存在大量错误；对Qwen2.5-VL视觉编码器的探测实验显示相机运动线索在深层ViT块中表征较弱；提出的注入方法显著改善了运动识别能力，使模型响应更加相机感知。

Conclusion: 几何驱动的线索提取和结构化提示是构建相机感知VideoLLM和VLA系统的实用步骤。该方法无需昂贵训练或微调，通过外部几何信息注入有效弥补了现有模型的不足。
...
TL;DR: FDeID-Toolbox：一个用于人脸去身份化研究的综合工具箱，解决该领域实现碎片化、评估协议不一致和结果不可比的问题。


<details>
  <summary>Details</summary>
Motivation: 人脸去身份化领域存在实现碎片化、评估协议不一致和结果不可比的问题，这源于任务本身的复杂性：FDeID涉及多个下游应用，需要在隐私保护、效用保持和视觉质量三个维度进行评估。

Method: 提出了FDeID-Toolbox，采用模块化架构，包含四个核心组件：标准化的主流基准数据集加载器、统一的方法实现（涵盖经典方法和SOTA生成模型）、灵活的推理管道、以及覆盖隐私、效用和质量指标的系统评估协议。

Result: 实验表明，FDeID-Toolbox能够在一致条件下实现对不同FDeID方法的公平和可重复比较。

Conclusion: FDeID-Toolbox为可重复的人脸去身份化研究提供了一个全面的工具箱，解决了该领域的关键挑战，促进了公平比较和可重复性研究。
...
TL;DR: 提出一种轻量级感知调度框架，利用先前帧输出估计和调度必要感知模块，在HRC中减少计算延迟27.52%，提升MMPose激活召回率72.73%，关键帧准确率达98%
...
Method: 受Relevance概念和HRC事件信息稀疏性启发，提出轻量级感知调度框架，利用先前帧输出实时估计和调度必要感知模块，基于场景上下文进行智能调度。

Result: 相比传统并行感知管道，计算延迟减少27.52%，MMPose激活召回率提升72.73%，关键帧准确率达到98%，验证了在不显著牺牲精度的情况下提升实时感知效率的能力。

Conclusion: 该框架展示了作为HRC中多模态流式感知系统的可扩展系统化解决方案的潜力，能够有效平衡感知质量和实时性能。
...
TL;DR: 该论文提出了世界场景图生成（WSGG）任务，旨在构建包含场景中所有交互对象（包括被遮挡或未观察到的对象）的世界场景图，并引入了ActionGenome4D数据集和三种互补的方法来解决未观察对象的推理问题。


<details>
  <summary>Details</summary>
Motivation: 现有时空场景图方法本质上是帧中心的：只推理当前可见对象，丢弃被遮挡的实体，并在2D空间中操作。这限制了场景理解的完整性和持续性，无法处理对象因遮挡或相机运动而暂时不可见的情况。

Method: 1. 引入ActionGenome4D数据集，将Action Genome视频升级为4D场景，包含每个对象的3D重建、世界坐标系下的定向边界框和密集关系标注（包括暂时未观察到的对象）。
2. 提出世界场景图生成（WSGG）任务，在每个时间戳构建包含场景中所有交互对象（包括观察和未观察对象）的世界场景图。
3. 提出三种互补方法：PWG（持久世界图）通过零阶特征缓冲区实现对象持久性；MWAE（掩码世界自编码器）将未观察对象推理重构为带跨视图关联检索的掩码补全；4DST（4D场景变换器）用可微分逐对象时间注意力替换静态缓冲区，并加入3D运动和相机姿态特征。
4. 设计并评估开源视觉语言模型在WSGG任务上的性能，通过基于图RAG的方法建立未定位关系预测的基线。

Result: 论文提出了WSGG任务框架和三种互补方法，建立了包含未观察对象推理的场景理解新范式。通过ActionGenome4D数据集和基于图RAG的方法，为未定位关系预测建立了基线，推动了视频场景理解向世界中心、时间持久和可解释的场景推理方向发展。

Conclusion: WSGG将视频场景理解从帧中心方法推进到世界中心、时间持久和可解释的场景推理。通过处理未观察对象的问题，该方法能够构建更完整、持续的时空场景表示，为高级视频理解和推理任务提供了更强大的基础。
...
TL;DR: STEVO-Bench是一个评估视频世界模型能否将状态演化与观测解耦的基准测试，通过遮挡、关灯、相机移开等观测控制来检验模型在无观测情况下的演化能力。
...
Method: 设计了STEVO-Bench基准测试，通过遮挡器插入、关灯、指定相机"移开"轨迹等观测控制指令来评估视频世界模型。该基准提出自动检测和分离视频世界模型在自然状态演化关键方面的失败模式的评估协议。

Result: 通过评估有相机控制和没有相机控制的视频模型在各种自然演化过程中的表现，揭示了它们在解耦状态演化与观测方面的局限性。STEVO-Bench结果分析为当前视频世界模型的潜在数据和架构偏差提供了新的见解。

Conclusion: STEVO-Bench为评估视频世界模型的解耦能力提供了系统化的基准测试，暴露了现有模型在无观测情况下维持状态演化的局限性，为改进视频世界模型提供了重要方向。
...
TL;DR: 提出Visual-ERM视觉等价奖励模型，通过多模态生成奖励为视觉到代码任务提供细粒度、可解释的视觉空间反馈，显著提升强化学习性能


<details>
  <summary>Details</summary>
Motivation: 现有视觉到代码任务中，强化学习面临奖励信号不对齐的问题。现有奖励要么依赖文本规则，要么使用粗糙的视觉嵌入相似度，都无法捕捉细粒度视觉差异且容易受到奖励攻击

Method: 提出Visual-ERM视觉等价奖励模型，这是一种多模态生成奖励模型，直接在渲染的视觉空间中评估视觉到代码的质量，提供细粒度、可解释、任务无关的反馈

Result: Visual-ERM将Qwen3-VL-8B-Instruct在图表到代码任务上提升+8.4分，在表格和SVG解析上分别平均提升+2.7和+4.1分；在VC-RewardBench基准测试中，8B参数的Visual-ERM明显优于Qwen3-VL-235B-Instruct，接近领先的闭源模型

Conclusion: 细粒度的视觉奖励监督对于视觉到代码的强化学习既是必要的也是充分的，无论任务特异性如何；Visual-ERM为视觉到代码任务提供了有效的强化学习解决方案
...
TL;DR: 提出了一种名为\method{}的知识蒸馏框架，通过训练轻量级探针在冻结的教师模型隐藏状态上，使用探针的预测而非输出logits作为学生模型的监督信号，从而绕过词汇投影瓶颈，在推理任务上获得更干净的蒸馏信号。


<details>
  <summary>Details</summary>
Motivation: 在推理任务中，大型语言模型的输出分布并不总是高质量的训练信号。模型的中间表示可能编码了正确答案，但通过词汇投影（提示格式化和答案标记选择）时这些信息会丢失或扭曲，导致输出脆弱且嘈杂。

Method: \method{}框架在冻结的教师模型隐藏状态上训练轻量级探针，然后使用探针的预测（而非教师模型的输出logits）作为学生模型的监督信号进行训练。这种方法不需要改变学生或教师模型的架构，探针训练成本低且教师表示可以缓存。

Result: 在四个推理基准测试（AQuA-RAT、ARC Easy/Challenge和MMLU）上获得了一致的改进，在有限数据条件下效果最为显著。探针在中间表示上训练的标签比教师自身输出更干净，有效地去噪了蒸馏信号。

Conclusion: 通过利用内部表示，\method{}使从业者能够从大型教师模型中提取更多价值，无需额外的训练数据或架构复杂性。该方法架构无关，计算开销小，为知识蒸馏提供了一种更有效的方法。
...
TL;DR: LLMs在知识更新场景中存在检索偏差问题，随着同一事实多次更新，模型更倾向于检索早期版本而非最新版本，类似于认知心理学中的AB-AC干扰现象。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注单次知识更新或单一冲突，而实际应用中同一事实可能在上下文中多次修订，多个历史有效版本在检索时相互竞争，这一问题尚未得到充分探索。

Method: 引入动态知识实例（DKI）评估框架，将同一事实的多次更新建模为线索与更新值序列的配对，通过端点探测（初始状态和最新状态）评估模型，并进行注意力、隐藏状态相似性和输出logits的诊断分析。

Result: 随着更新次数增加，检索偏差加剧：初始状态准确率保持高位，而最新状态准确率大幅下降；错误样本上的注意力、隐藏状态相似性和logits信号变得平坦且区分度弱；认知启发式干预策略仅带来有限改善。

Conclusion: LLMs在长上下文中跟踪和遵循知识更新存在持续挑战，检索偏差问题难以通过简单干预消除，需要更深入的研究来解决多更新场景下的知识管理问题。
...
TL;DR: ActTail是一种基于TopK幅度和重尾自正则化理论的激活稀疏化方法，通过投影特定的稀疏预算分配来提升大语言模型推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有激活稀疏化方法通常在投影间采用均匀稀疏度，忽略了Transformer权重的异质性统计特性，导致性能下降加剧。需要一种能考虑投影间统计差异的稀疏分配方法。

Method: 提出ActTail方法：基于TopK幅度的激活稀疏化，结合重尾自正则化理论进行全局激活稀疏分配。通过计算每个投影的经验谱密度获取重尾指数，作为投影特定稀疏预算的量化指标。理论分析建立了激活稀疏比与重尾指数之间的显式关系。
...
Conclusion: ActTail通过考虑投影间的统计异质性，基于重尾自正则化理论提供原则性的稀疏分配指导，显著提升了激活稀疏化方法的性能，为大语言模型推理加速提供了更有效的解决方案。
...
TL;DR: 通过自蒸馏方法从多轮用户对话中学习，利用后续用户消息作为反馈信号改进语言模型，实现对齐、个性化和持续适应


<details>
  <summary>Details</summary>
Motivation: 语言模型产生的大量多轮用户交互数据通常被丢弃，但这些交互包含有价值的信息：后续用户消息可能表明模型回答不正确、未遵循指令或不符合用户偏好。模型本身已经能够在上下文中利用这些信息进行行为修正，这为学习提供了机会。

Method: 提出基于自蒸馏的原则性可扩展方法：1）将模型条件化于用户的后续消息，获得后见之明分布；2）比较该分布与原始策略的差异，获得更新目标；3）将后见之明分布蒸馏回当前策略。使用WildChat的真实用户对话数据进行训练。

Result: 在标准对齐和指令遵循基准测试中，模型性能得到改善，且未损害其他能力。同一机制还支持个性化，使模型能够通过交互持续适应个体用户，无需显式反馈。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在记忆和处理大规模训练数据方面表现出前所未有的能力，这引发了安全、隐私和知识产权方面的严重问题。现有遗忘学习方法主要关注平面句子级数据，忽视了自然结构化数据中的关系型、多跳推理知识，因此需要专门针对知识图谱事实的遗忘学习评估基准和方法。

Method: 提出了Graph Oblivion and Node Erasure (GONE)基准，用于评估大语言模型在结构化知识图谱事实上的遗忘学习效果，能够分离三种遗忘效应：直接事实移除、推理泄漏和灾难性遗忘。同时设计了Neighborhood-Expanded Distribution Shaping (NEDS)框架，利用图连接性识别锚点相关邻居，在遗忘事实与其语义邻域之间建立精确决策边界。

Result: 在LLaMA-3-8B和Mistral-7B模型上评估多种知识编辑和遗忘学习方法，NEDS在GONE基准上表现出优越性能：遗忘效能达到1.000，局部性达到0.839，在其他基准上也表现优异。

Conclusion: GONE基准填补了结构化知识遗忘学习评估的空白，NEDS框架通过利用图连接性和语义邻域信息，实现了对知识图谱事实的有效遗忘学习，为解决大语言模型中的安全、隐私和知识产权问题提供了新思路。
...
TL;DR: 论文揭示了语言模型易受提示注入攻击的根本原因：角色混淆。模型通过文本写作方式而非来源判断角色身份，导致恶意文本模仿特定角色即可获得该角色的权限。


<details>
  <summary>Details</summary>
Motivation: 尽管进行了广泛的安全训练，语言模型仍然容易受到提示注入攻击。本研究旨在探究这种脆弱性的根本原因，特别是模型如何内部识别"谁在说话"以及为什么恶意文本能够获得不应有的权限。

Method: 设计了新颖的角色探针来捕捉模型内部如何识别说话者角色。通过向用户提示和工具输出中注入伪造的推理过程来测试角色混淆机制，在多个开源和闭源模型上进行实验。

Result: 在StrongREJECT和智能体数据泄露任务上，攻击平均成功率分别达到60%和61%，而基线接近零。内部角色混淆程度在生成开始前就能强烈预测攻击成功率。

Conclusion: 研究揭示了语言模型安全性的根本缺陷：安全定义在接口层面，但权限分配发生在潜在空间中。所有提示注入攻击都利用了相同的底层角色混淆机制，这为理解提示注入提供了统一的机制性框架。
...
TL;DR: TASTE-S：一种可流式化的文本-语音联合口语建模方法，通过集成CTC-ASR模块和重新设计单元解码器，在保持性能的同时显著降低延迟，支持实时使用。


<details>
  <summary>Details</summary>
Motivation: 现有文本-语音联合口语建模（SLM）系统存在模态不匹配问题（语音单元序列远长于文本标记），且依赖外部ASR系统和非因果解码器，限制了流式使用。

Method: 提出TASTE-S方法：1）在编码器中集成基于CTC的ASR模块，实现即时双模态编码；2）重新设计单元解码器，支持实时解码；3）采用联合训练。

Result: TASTE-S在性能上与TASTE相当，同时显著降低延迟；进一步研究表明，TASTE-S对转录保持鲁棒性，并支持长格式编码和解码。

Conclusion: TASTE-S成功解决了现有文本-语音联合建模方法的流式化限制，为实时语音交互系统提供了可行的技术方案。
...
TL;DR: 研究发现思维链（CoT）训练对模型行为有因果影响，即使最终答案相同，不同的推理路径也会导致不同的有害行为模式，挑战了仅监督输出的对齐策略。


<details>
  <summary>Details</summary>
Motivation: 思维链常被视为窥探LLM决策的窗口，但近期研究表明它可能只是事后合理化。这引发了一个关键的对齐问题：推理轨迹是否独立于最终答案对模型泛化产生因果影响？

Method: 设计控制实验，保持最终有害答案不变但改变推理路径。构建包含邪恶推理、误导性推理和顺从性推理的数据集，在多种范式下训练模型（0.6B-14B参数），包括QTA、QT和T-only，并在有思考和无思考模式下评估。

Result: 1) CoT训练比标准微调更能放大有害泛化；2) 不同推理类型尽管最终答案相同，但会引发与其语义一致的不同行为模式；3) 仅训练推理而无答案监督（QT或T-only）足以改变行为，证明推理携带独立信号；4) 这些效应在无推理生成答案时仍持续，表明深度内化。

Conclusion: 推理内容具有因果效力，挑战了仅监督输出的对齐策略。模型会内化训练中使用的推理模式，即使最终答案相同，不同的推理路径也会导致不同的行为泛化。
...
TL;DR: ShatterMed-QA是一个包含10,558个多跳临床问题的双语基准测试，旨在评估深度诊断推理能力。通过k-粉碎算法构建拓扑正则化医学知识图谱，剪除通用枢纽节点以切断逻辑捷径，并通过桥接实体掩码和拓扑驱动负采样合成评估案例。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在标准医学基准测试中表现优异，但在真实临床环境中所需的复杂多跳诊断推理方面严重不足。主要障碍是"捷径学习"问题，模型利用知识图谱中的通用枢纽节点绕过真实的微观病理级联推理。

Method: 1. 使用新颖的k-粉碎算法构建拓扑正则化医学知识图谱，物理剪除通用枢纽节点以显式切断逻辑捷径；2. 通过隐式桥接实体掩码和拓扑驱动硬负采样合成评估案例，强制模型在不依赖表面消除的情况下导航生物学上合理的干扰项。

Result: 对21个大型语言模型的综合评估显示，在多跳任务上性能大幅下降，特别是领域特定模型。通过检索增强生成恢复掩码证据后，几乎所有模型都实现了性能恢复，验证了ShatterMed-QA的结构保真度，并证明了其在诊断当前医学AI基本推理缺陷方面的有效性。

Conclusion: ShatterMed-QA基准测试成功揭示了当前医学AI在多跳诊断推理方面的根本缺陷，通过切断知识图谱中的逻辑捷径，强制模型进行真实的病理级联推理。该框架为评估和改进医学AI的深度推理能力提供了有效工具。
...
TL;DR: 研究显示主流LLMs在为学生提供写作反馈时，会根据学生的性别、种族、学习需求等属性产生系统性、刻板印象对齐的反馈偏差，即使面对相同的作文内容。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM驱动的工具承诺大规模自动化个性化反馈，但LLMs并非语言中立：它们偏向标准学术英语并复制社会刻板印象，引发了对"个性化"如何影响学生接收反馈的担忧。

Method: 使用PERSUADE数据集中600篇八年级说服性作文，在提示条件中嵌入性别、种族/民族、学习需求、成就和动机等学生属性，生成四种主流LLM的反馈，通过改编Marked Words框架分析词汇变化。

Result: 结果显示，即使作文内容相同，基于预设学生属性的反馈存在系统性、刻板印象对齐的变化。对种族、语言或残疾标记的学生，反馈常表现出积极反馈偏差和反馈保留偏差——过度使用表扬、较少实质性批评、能力有限假设。不同属性下，模型不仅调整强调的内容，还改变写作评判方式和学生称呼方式。
...
TL;DR: LMEB是一个新的长时记忆嵌入基准测试，填补了现有文本嵌入基准在评估长时程、碎片化、上下文依赖记忆检索能力方面的空白。


<details>
  <summary>Details</summary>
Motivation: 当前文本嵌入基准主要关注传统段落检索，未能充分评估模型处理长时程记忆检索任务的能力，这些任务涉及碎片化、上下文依赖和时间上遥远的信息。

Method: 提出了长时程记忆嵌入基准(LMEB)，包含22个数据集和193个零样本检索任务，涵盖4种记忆类型：情景记忆、对话记忆、语义记忆和程序记忆，包含AI生成和人工标注数据。

Result: 评估了15个广泛使用的嵌入模型，发现：(1) LMEB提供了合理的难度水平；(2) 更大的模型并不总是表现更好；(3) LMEB与MTEB存在正交性，表明传统段落检索性能不能泛化到长时程记忆检索。

Conclusion: LMEB填补了记忆嵌入评估的关键空白，提供了标准化、可复现的评估框架，推动文本嵌入在处理长期、上下文依赖记忆检索方面的进一步发展。
...
TL;DR: 提出Expert Pyramid Tuning (EPT)，一种参数高效微调方法，通过多尺度特征金字塔结构解决现有MoE-LoRA方法忽视任务层次复杂性的问题，在减少参数的同时提升多任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于Mixture-of-Experts的LoRA变体虽然通过动态路由取得了不错效果，但忽视了任务复杂度的层次性。这些方法通常使用统一架构的专家，限制了它们捕捉不同任务所需多样化特征粒度的能力——有些任务需要高级语义抽象，而另一些则需要细粒度句法操作。

Method: 提出Expert Pyramid Tuning (EPT)，将计算机视觉中的多尺度特征金字塔概念引入PEFT领域。方法包括：(1) 共享元知识子空间，在低维度编码通用语言模式；(2) 金字塔投影机制，使用可学习的上投影算子在不同尺度重建高维特征；(3) 任务感知路由器动态选择这些多尺度特征的最优组合。

Result: 在多个多任务基准测试上的广泛实验表明，EPT显著优于最先进的MoE-LoRA变体。关键的是，得益于设计的重参数化能力，EPT在实现性能提升的同时减少了训练参数数量。

Conclusion: EPT通过引入多尺度特征金字塔结构，有效解决了现有参数高效微调方法忽视任务层次复杂性的问题，在减少参数的同时提升了多任务性能，为LLM在多任务场景中的部署提供了更高效的解决方案。
...
TL;DR: RTD-Guard是一个无需对抗数据、模型调优或白盒访问的黑盒文本对抗样本检测框架，利用预训练的替换令牌检测器定位可疑扰动，通过两次查询观察预测置信度变化来检测攻击。


<details>
  <summary>Details</summary>
Motivation: 现有对抗样本检测方法通常需要攻击的先验知识、对受害者模型的白盒访问或大量查询，这严重限制了实际部署。需要一种轻量级、实用且资源友好的检测机制，特别适合资源受限或隐私敏感环境。

Method: RTD-Guard利用预训练的替换令牌检测(RTD)判别器定位可疑令牌，无需微调；然后掩码这些可疑令牌，通过观察受害者模型在干预前后的预测置信度变化来检测对抗样本。整个过程仅需两次黑盒查询。

Result: 在多个基准数据集上的综合实验表明，RTD-Guard能有效检测由多种最先进攻击方法生成的对抗文本，在多个指标上超越现有检测基线，提供高效、实用且资源轻量的防御机制。

Conclusion: RTD-Guard为文本对抗样本检测提供了一种新颖的黑盒框架，特别适合实际部署，无需对抗数据、模型调优或内部模型访问，仅需两次查询，在资源受限或隐私敏感环境中具有显著优势。
...
TL;DR: SCILIRE是一个基于人机协作原则设计的科学文献结构化知识提取系统，通过迭代式工作流程让研究人员审查和修正AI输出，并利用这些反馈改进后续的LLM推理，从而提高提取准确性和数据集创建效率。
...
Method: 设计了基于人机协作原则的SCILIRE系统，采用迭代式工作流程，研究人员可以审查和修正AI输出，并将这些交互作为反馈信号来改进未来的LLM推理。

Result: 通过内在基准测试和跨多个领域的实际案例研究评估，结果表明SCILIRE提高了提取准确性并促进了高效的数据集创建。
...
TL;DR: 这篇综述系统梳理了面向大语言模型的持续学习方法，从三个训练阶段（持续预训练、持续微调、持续对齐）和三类方法（基于排练、正则化、架构）进行分类分析，强调了大语言模型持续学习与传统机器学习的核心差异。


<details>
  <summary>Details</summary>
Motivation: 大语言模型需要动态适应不断演化的知识和序列任务，但传统的静态预训练范式存在灾难性遗忘问题。持续学习成为解决这一关键限制的重要范式，使大语言模型能够在保持已有知识的同时学习新知识。

Method: 1. 围绕三个核心训练阶段构建分类框架：持续预训练、持续微调、持续对齐；2. 在传统的排练、正则化、架构三类方法基础上，进一步根据遗忘缓解机制细分；3. 对传统持续学习方法在大语言模型上的适应性和关键改进进行严格比较分析；4. 明确强调大语言模型持续学习与传统机器学习的核心差异。

Result: 当前方法在特定领域展现出有希望的结果，但在实现跨多样任务和时间尺度的无缝知识整合方面仍存在根本性挑战。分析涵盖了遗忘率、知识转移效率等关键评估指标，以及评估持续学习性能的新兴基准。

Conclusion: 这篇系统综述为大语言模型适应性的研究提供了结构化框架，帮助研究人员和实践者理解当前成就和未来机会，为语言模型的终身学习研究做出贡献。
...
TL;DR: TESS提出了一种通过时间演化语义空间来桥接文本与时间序列预测模态差距的新方法，在四个真实数据集上实现了高达29%的预测误差降低。


<details>
  <summary>Details</summary>
Motivation: 现有方法在将文本信息融入时间序列预测时面临模态差距问题：文本描述以隐式和定性方式表达时间影响，而预测模型需要显式和定量信号。现有方法容易过度关注冗余标记，难以可靠地将文本语义转化为可用的数值线索。
...
Conclusion: TESS通过时间演化语义空间有效桥接了文本与时间序列预测之间的模态差距，显著提升了预测性能，同时保持了可解释性。
...
TL;DR: 研究发现ChatGPT模型在递归训练合成数据后出现模型自我收敛现象，导致文本生成多样性下降


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在递归训练合成数据时容易发生模型崩溃，现有研究多从理论或实证单一角度分析，缺乏对模型输出质量随时间变化的纵向研究

Method: 使用文本相似性度量方法评估不同ChatGPT模型生成多样化文本输出的能力，通过设置温度参数为1来明确要求生成多样化文本

Result: 近期ChatGPT版本生成多样化文本的能力出现可测量的下降，即使明确要求生成多样化文本，输出多样性仍然减少

Conclusion: 模型自我收敛现象导致不同ChatGPT版本生成文本的相似性逐渐增加，这可能源于训练数据中合成数据比例的增加，反映了LLM生成数据对互联网的渗透影响
...
TL;DR: 提出基于解决方案条件和对抗性验证的框架，通过迭代优化测试用例构建EvolveCoder-22k数据集，显著提升代码生成强化学习效果


<details>
  <summary>Details</summary>
Motivation: 现有代码生成强化学习中的验证信号较弱且静态，限制了强化学习在代码生成中的有效性

Method: 提出解决方案条件和对抗性验证框架，基于候选解决方案的执行行为迭代优化测试用例，增加难度、提高区分度、减少冗余，并构建EvolveCoder-22k数据集

Result: 迭代优化显著增强验证效果，pass@1从43.80降至31.22；在EvolveCoder-22k上的强化学习稳定优化，Qwen3-4B在四个下游基准上平均提升4.2分，优于其他4B规模基线
...
TL;DR: 研究首次评估大型语言模型对伊斯兰教逊尼派和什叶派两大教派的处理差异，发现模型存在基于语言和地理位置的不一致性偏见。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型成为宗教知识的重要来源，需要评估其是否公平对待不同宗教群体，特别是伊斯兰教两大主要教派之间的差异。

Method: 开发了SectEval测试集（包含88个问题，支持英语和印地语），评估了15个顶级专有和开源模型，分析语言和地理位置对模型回答的影响。

Result: 发现显著的基于语言的不一致性：英语环境下DeepSeek-v3和GPT-4o等模型偏向什叶派答案，而印地语环境下同一模型转向偏向逊尼派答案。地理位置也影响回答：Claude-3.5等高级模型会根据用户国家调整答案（如伊朗用户得到什叶派答案，沙特用户得到逊尼派答案），而较小模型则坚持逊尼派观点。

Conclusion: AI并非中立，其宗教"真相"会根据用户使用的语言和声称来自的国家而变化，这揭示了大型语言模型在处理宗教知识时存在的系统性偏见问题。
...
TL;DR: SteerRM是一种无需训练的去偏方法，使用稀疏自编码器干预来减少奖励模型对表面风格线索的偏见，保持整体性能的同时提升硬分割准确率。


<details>
  <summary>Details</summary>
Motivation: 奖励模型在对齐流程中至关重要，但它们存在对表面风格线索的偏见，倾向于选择格式更好而非语义更优的回答。现有去偏方法通常需要重新训练或架构修改，而直接激活抑制会因表示纠缠导致性能下降。

Method: SteerRM使用稀疏自编码器干预：1）通过对比配对响应隔离风格效应；2）使用强度-稳定性准则识别偏见相关SAE特征；3）在推理时抑制这些特征。这是首个无需训练的去偏方法。

Result: 在RM-Bench的六个奖励模型上，SteerRM平均提升硬分割准确率7.3个百分点，同时保持整体性能。在Gemma-based奖励模型和受控非格式偏见上的结果表明该方法能泛化到不同架构和偏见类型。

Conclusion: SAE-based干预可以在不重新训练的情况下缓解奖励模型偏见，为对齐流程提供实用且可解释的解决方案。研究发现格式相关特征集中在浅层并在模型间转移，揭示了共享的架构级偏见编码模式。
...
TL;DR: 本文研究了强化学习可验证奖励（RLVR）中多选题选项设计的影响，发现选项数量不匹配和强干扰项的重要性，并提出迭代干扰项筛选（IDC）框架来提升RLVR训练效果。


<details>
  <summary>Details</summary>
Motivation: 多选题为RLVR提供了可扩展的验证数据源，但存在奖励黑客风险（如随机猜测或简单排除）。现有方法通常将多选题转换为开放式问题，从而丢弃了专家设计的干扰项提供的对比信号。

Method: 系统研究选项设计对RLVR的影响，提出迭代干扰项筛选（IDC）框架，主动构建高质量干扰项以阻止排除捷径并促进深度推理。

Result: 实验表明：1）训练和测试中选项数量不匹配会降低性能；2）强干扰项能有效缓解随机猜测，即使只有2个选项也能实现有效的RLVR训练；3）IDC方法显著提升了干扰项质量，在多个基准测试中相比原始数据获得了显著的RLVR训练增益。

Conclusion: 选项设计对RLVR至关重要，通过精心设计的干扰项可以缓解奖励黑客问题，IDC框架能够有效提升RLVR训练效果，为利用多选题进行强化学习提供了新的方法。
...
Method: 开发了欧洲葡萄牙语LLM排行榜，创建了包含模型安全性和葡萄牙文化对齐等新维度的基准测试套件。

Result: 建立了首个专门针对欧洲葡萄牙语的LLM排行榜，提供了包含新颖评估维度的基准测试，并在Hugging Face平台上公开可用。

Conclusion: 该研究填补了欧洲葡萄牙语LLM评估的空白，为开发者和研究者提供了重要的评估工具，促进了该语言变体的大语言模型发展。
...
TL;DR: 该研究系统性地将BabyBERTa扩展到英法双语场景，在严格数据量匹配条件下比较了单语、双语和跨语言设置，发现训练数据类型对句法和语义任务有不同影响：维基百科数据对语义任务有益，儿童导向语音对单语语法判断有益，双语预训练对文本蕴含任务（特别是法语）有显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有发展合理的语言模型研究主要集中在英语上，对于多语言环境下的情况存在开放性问题。研究者希望系统性地研究紧凑语言模型在英法双语场景下的表现，特别是在严格数据量匹配条件下。

Method: 扩展BabyBERTa到英法双语场景，在严格数据量匹配条件下比较三种设置：单语、双语和跨语言。对比两种训练语料：1)儿童导向语音（约250万词元），遵循BabyBERTa和相关工作；2)多领域语料（约1000万词元），将BabyLM框架扩展到法语。引入新资源包括法语版本的QAMR和QASRL，以及英法多领域语料。

Result: 结果显示出情境依赖效应：维基百科训练始终对语义任务有益，而儿童导向语音在单语设置中改善语法判断。双语预训练在文本蕴含任务上带来显著提升，特别是对法语有明显改善。重要的是，在BabyBERTa、RoBERTa和LTG-BERT中都出现了相似的模式，表明这些趋势在不同架构中具有一致性。

Conclusion: 该研究为多语言发展合理的语言模型提供了系统性分析，揭示了训练数据类型对模型性能的差异化影响，并展示了双语预训练在特定任务上的优势。研究结果在不同模型架构中具有一致性，为未来多语言语言模型的发展提供了重要见解。
...
TL;DR: DS²-Instruct：零样本框架，无需人工标注即可生成领域特定指令数据集，通过任务关键词生成、布鲁姆分类法认知层级配对和自一致性验证来确保数据质量和领域覆盖
...
Method: DS²-Instruct框架：1) 生成任务相关关键词确保全面领域覆盖；2) 将关键词与布鲁姆分类法的不同认知层级配对创建多样化指令；3) 使用自一致性验证确保数据质量。

Result: 在数学、金融、逻辑推理等七个挑战性领域生成数据集，评估显示使用该数据微调的模型相比现有数据生成方法有显著改进。

Conclusion: DS²-Instruct提供了一种无需人工监督的领域特定指令数据集生成方法，能有效提升LLM在专业领域的适应能力。
...
TL;DR: Long-form RewardBench是首个专门针对长文本生成的奖励建模测试平台，包含QA、RAG、Chat、Writing和Reasoning五个子任务，评估了20多个主流奖励模型，发现当前模型在长文本奖励建模能力上仍有不足。
...
Method: 通过精心设计的多阶段数据收集过程收集指令和偏好数据，构建包含五个关键子任务的长文本奖励建模测试平台，并对20多个主流奖励模型（包括分类器和生成模型）进行广泛实验，还设计了新颖的"长文本针在干草堆测试"。

Result: 研究发现当前模型仍缺乏长文本奖励建模能力；长文本针在干草堆测试揭示了奖励建模性能与错误在响应中的位置以及整体响应长度之间的相关性，分类器和生成模型表现出不同的特性；分类器比在相同数据上训练的生成模型具有更好的泛化能力。

Conclusion: 作为首个长文本奖励建模基准，这项工作旨在为这一关键领域的进展可视化提供稳健平台，填补了长文本生成奖励模型评估的重要空白。
...
TL;DR: 提出基于最小贝叶斯风险解码的自进化框架，无需人工标注即可训练错误跨度检测模型，性能超越基于人工标注的基线方法


<details>
  <summary>Details</summary>
Motivation: 机器翻译评估中的错误跨度检测任务依赖昂贵且标注不一致的人工标注数据，需要寻找替代方案

Method: 提出迭代MBR蒸馏框架，利用现成的大语言模型生成伪标签，通过最小贝叶斯风险解码进行自进化训练

Result: 在WMT指标共享任务数据集上的实验表明，仅使用自生成伪标签训练的模型在系统和跨度级别上优于未适应基础模型和基于人工标注的监督基线，同时保持竞争力的句子级性能

Conclusion: 提出的自进化框架能够有效替代人工标注，为错误跨度检测任务提供了一种高效且可扩展的解决方案
...
TL;DR: 本文提出了一种PCA扫描方法来解决监督语义差异分析中PCA维度选择的问题，通过联合考虑表示能力、梯度可解释性和稳定性来选择最优维度，减少了研究自由度。


<details>
  <summary>Details</summary>
Motivation: 监督语义差异分析在应用PCA时缺乏系统的方法来选择保留的主成分数量，这引入了可避免的研究自由度，影响了分析结果的稳定性和可解释性。

Method: 提出PCA扫描程序，将维度选择视为表示能力、梯度可解释性和K值附近稳定性三个标准的联合优化问题。通过在不同PCA维度下评估这些标准来选择最优K值。

Result: 在关于人工智能的短文本语料库中，PCA扫描方法产生了稳定、可解释的"钦佩"相关梯度，区分了乐观协作与不信任嘲讽的AI话语框架，而"竞争"维度未产生稳健对齐。对比实验显示，高PCA维度启发式方法会产生分散、弱结构的聚类。

Conclusion: PCA扫描方法在约束研究自由度的同时保留了监督语义差异分析的解释性目标，支持对内涵意义进行透明且具有心理学意义的分析。
...
Method: 提出WALAR方法，利用单语文本进行强化训练。通过观察现有基于源语言的多语言质量评估模型的失败模式，开发了词对齐和语言对齐技术来缓解这些"漏洞"，从而改进强化学习的奖励机制。

Result: 使用WALAR持续训练了一个支持101种语言翻译的大语言模型。实验表明，该模型在Flores-101数据集的1400个语言方向上大幅超越了最强的开源多语言大语言模型LLaMAX。

Conclusion: WALAR方法有效提升了低资源语言翻译能力，同时保持高资源语言性能，为解决低资源语言翻译数据稀缺问题提供了新思路。
...
TL;DR: 本文介绍了ESG-Bench基准数据集，用于评估大语言模型在ESG报告理解方面的能力，并减少模型幻觉。该数据集包含基于真实ESG报告的人工标注问答对，通过特定设计的思维链提示策略，显著提升了模型在ESG领域的事实准确性和推理能力。


<details>
  <summary>Details</summary>
Motivation: 随着ESG（环境、社会和治理）报告在许多地区成为法律要求，这些报告的长度和复杂性使得解读和自动化分析变得困难。需要支持可扩展且可信赖的分析工具，特别是在社会敏感、合规关键的环境中减少模型幻觉。

Method: 1. 创建ESG-Bench基准数据集，包含基于真实ESG报告的人工标注问答对，带有细粒度标签指示模型输出是否得到事实支持或存在幻觉。2. 将ESG报告分析框架化为具有可验证性约束的问答任务。3. 设计任务特定的思维链（CoT）提示策略。4. 使用CoT标注的推理过程微调多个最先进的大语言模型。

Result: 实验表明，基于思维链的方法在减少幻觉方面显著优于标准提示和直接微调。这些改进不仅限于ESG领域，还能迁移到现有的问答基准测试中，显示出方法的泛化能力。

Conclusion: ESG-Bench为ESG报告理解提供了一个系统化的评估框架，通过思维链方法有效减少了大语言模型在合规关键环境中的幻觉问题，为社会责任敏感领域的可靠自动化分析提供了新途径。
...
TL;DR: NAIT框架通过分析神经元激活模式相似性，从指令调优数据集中选择高效子集，仅用10%数据就能超越传统方法，并揭示了激活特征的可迁移性。


<details>
  <summary>Details</summary>
Motivation: 指令调优数据过多会降低大语言模型性能，而精心选择高质量数据子集能显著提升能力。如何从指令调优数据集中识别最有效的子集来开发特定或通用能力成为关键挑战。

Method: 提出NAIT框架，通过分析IT数据与目标领域能力之间的神经元激活模式相似性来评估数据影响。从目标能力领域内数据捕获神经元激活模式构建可重用、可迁移的激活特征，然后基于候选样本与目标能力预期激活特征的相似性评估和选择最优样本。

Result: 实验表明，使用NAIT选择的10% Alpaca-GPT4 IT数据子集训练，在各种任务上持续优于依赖外部高级模型或基于不确定性特征的方法。发现神经元激活特征在不同LLM能力间具有可迁移性：具有更多逻辑推理和编程特征的IT数据具有强通用可迁移性，而稳定的核心数据子集足以持续激活基础模型能力并普遍提升多样化任务性能。

Conclusion: NAIT提供了一种高效选择指令调优数据子集的方法，通过神经元激活模式相似性分析，能够显著提升模型性能并揭示数据特征的可迁移性规律，为优化大语言模型训练提供了新思路。
